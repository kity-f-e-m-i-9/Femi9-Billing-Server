...
class MonthlyTargetCalculator
{
    ...
    private function calculateMonthlySales(
        ...
        // Calculate sales from user_invoice table (B2B)
        $stmt = $this->conn->prepare("
        ...
        // Calculate sales from invoice table (B2C)
        $stmt = $this->conn->prepare("
            SELECT COALESCE(SUM(CAST(total AS DECIMAL(10,2))), 0) AS total_sales
            FROM invoice
            WHERE user_type = ?
        ...
        if (!$stmt) {
            throw new RuntimeException("Failed to prepare invoice query: " . $this->conn->error);
        }
        ...
        if ($row = $result->fetch_assoc()) {
            $sales['invoice_sales'] = (float)$row['total_sales'];
        }
        $stmt->close();
        
        // Calculate total
        $sales['total'] = $sales['user_invoice_sales'] + $sales['invoice_sales'];
    ...
    private function getSubordinateDistributors(string $parentType, string $parentId): array
    {
        ...
        foreach ($distributorTypes as $distType) {
            $parentColumn = 'stockiest_id'; // Both distributor and super_distributor use stockiest_id
