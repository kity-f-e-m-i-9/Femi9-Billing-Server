...
xmlhttp.send();}
			...
			<script>
			// Auto-suggest next invoice number (DTSH/FY/LLP{n} napkin /
			// DTSHD/FY/LLP{n} diaper). Suggested on page load (napkin default,
			// since no product is picked yet) and re-suggested once a product
			// is selected via showPrice() below — see invNumberSuggest(). Only
			...
			function invNumberSuggest(productId) {
			    ...
			    var url = 'load_next_invoice_number.php' + (productId ? '?pid=' + encodeURIComponent(productId) : '');
			    fetch(url)
