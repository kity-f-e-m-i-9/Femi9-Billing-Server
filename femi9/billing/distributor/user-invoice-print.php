...
if(!$result_Invoice_Details) {
    die("Error: Invoice not found with ID: " . htmlspecialchars($Invoice_ID));
}
...
if(!$result_UserProfiles) {
    die("Error: User profile not found for user: $Login_user_IDvl, type: $Login_user_TYPEvl");
}

// Determine which table to fetch seller details from based on logged-in user type
$seller_table = "";
if($Login_user_TYPEvl == "super_stockiest") {
    $seller_table = "super_stockiest";
} else if($Login_user_TYPEvl == "stockiest") {
    $seller_table = "stockiest";
} else if($Login_user_TYPEvl == "super_distributor") {
    $seller_table = "super_distributor";
} else if($Login_user_TYPEvl == "distributor") {
    $seller_table = "distributor";
} else if($Login_user_TYPEvl == "shop") {
    $seller_table = "shop";
} else {
    die("Error: Invalid seller user type: " . htmlspecialchars($Login_user_TYPEvl));
}

$select_UserdETAILS = "SELECT * FROM " . $seller_table . " WHERE temp_id='$Login_user_IDvl'";
$fetch_UserdETAILS = mysqli_query($db_conn, $select_UserdETAILS);

if(!$fetch_UserdETAILS) {
    die("Database Error: " . mysqli_error($db_conn) . "<br>Query: " . htmlspecialchars($select_UserdETAILS));
}

$result_UserdETAILS = mysqli_fetch_array($fetch_UserdETAILS);

if(!$result_UserdETAILS) {
    die("Error: Seller details not found in table '$seller_table' for user: " . htmlspecialchars($Login_user_IDvl));
}
...
// Set table name based on buyer user type
$tablename = "";
if($getinvuser == "stockiest") {
    ...
    die("Error: Invalid buyer user type: " . htmlspecialchars($getinvuser));
}

$customer_id = $result_Invoice_Details['to_user_id'];

// Validate customer_id before query
if(empty($customer_id)) {
    die("Error: Customer ID is empty");
}
...
// Check if query was successful
if(!$fetch_Customer_Details) {
    die("Database Error: " . mysqli_error($db_conn) . "<br>Query: " . htmlspecialchars($select_Cusotmer_Details));
}
...
if(!$result_Customer_Details) {
    die("Error: Customer not found in table '$tablename' with temp_id: " . htmlspecialchars($customer_id));
}
...
$buyer_business_address = $result_Customer_Details['address']; // This is from stockiest/distributor/etc table

// STEP 2: Get buyer's DELIVERY ADDRESS from users_profile (for Consignee To)
$select_userprofile = "SELECT companyname, deliveryaddress 
                       FROM users_profile 
                       WHERE user_tempid='$customer_id' 
                       AND usertype='$getinvuser'";
...
<br/>
<button type="button" onClick="javascript:window.location='user-invoice-add.php?invuser=<?=$getinvuser;?>';" class="btn btn-success m-b-xs m-r-xs" id="butonwidth">+ New Invoice</button>

<br/>
<button type="button" onClick="javascript:window.location='user-manage-invoice.php?invuser=<?=$getinvuser;?>';" class="btn btn-primary m-b-xs m-r-xs" id="butonwidth">Manage Invoice</button>
...
<!-------------Currency----------------->
<?php
$get_ccode = isset($_REQUEST['crcode']) ? $_REQUEST['crcode'] : 'Default';

if($get_ccode == "Default" || $get_ccode == NULL) {
    $Currency_symbol = "&#8377;";
    $Currency_Name = "INR";
    $result_currency223 = null;
} else {
    $get_ccode = base64_decode($get_ccode);
...
<select name="currency_code" class="form-control" style="padding:5px;width:180px;margin-left:-10px;" id="currencySelect">
			<?php if($get_ccode == NULL || $get_ccode == 'Default') { ?>
			...
			<?php 
            $select_currency = "SELECT * FROM country WHERE currency_name!='' ORDER BY c_name ASC";
...
<?php 
// Fetch stock request for delivery address
...
if($resultstockreq && $resultstockreq['delivery_address'] != NULL) {
    $deliveryaddress = $resultstockreq['delivery_address'];
} else if($result_userprofile && $result_userprofile['deliveryaddress'] != NULL) {
    $deliveryaddress = $result_userprofile['deliveryaddress'];
} else {
...
<b><?php echo $result_userprofile['companyname']; ?></b><br/>
GSTIN: <?php echo $result_Customer_Details['gstin']; ?><br/>
Mobile: <?php echo $result_Customer_Details['mobile_number']; ?><br/>
...
GSTIN: <?php echo $result_Customer_Details['gstin']; ?><br/>
Mobile: <?php echo $result_Customer_Details['mobile_number']; ?><br/>
<?php echo $buyer_business_address; ?><br/>
State: <?php echo $state_name; ?>, District: <?php echo $district_name; ?>
</p>
...
<td></td>
<td></td><td></td>
<td id="rightlaign"><b><?=$Currency_symbol;?>&nbsp;<?php echo number_format($discountamount, 2, '.', '');?></b></td>
</tr>
<?php } ?>
...
<?php
// Tax amount to words conversion
