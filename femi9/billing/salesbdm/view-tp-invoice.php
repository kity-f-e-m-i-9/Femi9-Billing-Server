...
$tpIdList = !empty($tpIds) ? implode(',', array_map('intval', $tpIds)) : '0';

$enc_id = $_GET['id'] ?? '';
$inv_id = (int) base64_decode($enc_id);
if (!$inv_id) { header("Location: tp-purchase-order.php"); exit; }

// Restrict to invoices belonging to a TP inside this BDM's own districts.
$stmt = $db_conn->prepare("
    ...
    WHERE tpi.id = ? AND tpi.territory_partner_id IN ($tpIdList)
");
$stmt->bind_param('i', $inv_id);
$stmt->execute();
$inv = $stmt->get_result()->fetch_assoc();
$stmt->close();
if (!$inv) { header("Location: tp-purchase-order.php"); exit; }
...
<head>
    ...
    <title>Invoice <?php echo htmlspecialchars($inv['invoice_number']); ?> : <?php echo $business_name; ?></title>
        ...
        <div class="app-content">
            <div class="content-wrapper">
                <div class="container-fluid">
                            ...
                            <div class="page-description">
                                <h1>
                                    <table class="headertble"><tr>
                                        <td>Invoice</td>
                                        <td><a href="tp-purchase-order.php" title="All Purchase Orders">&#9776;</a></td>
                                    </tr></table>
                                </h1>
                            </div>
                        </div>
                                ...
                                <div class="card-body">
                                    <table class="table table-sm" style="font-size:13px;">
                                        ...
                                        <tfoot>
                                            ...
                                            <?php if ($discount > 0): ?>
                                            <tr>
                                                <td colspan="5" class="text-right" style="color:#64748b;">Discount</td>
                                                <td class="text-right" style="color:#64748b;">&minus;&#8377;<?php echo inr_format($discount, 2); ?></td>
                                            </tr>
                                            <?php endif; ?>
                                            <?php if ($courier > 0): ?>
                                            <tr>
                                                <td colspan="5" class="text-right" style="color:#64748b;">
                    ...
                    <div class="row mb-3">
                        <div class="col">
                            <a href="tp-purchase-order.php" class="btn btn-secondary">
                                <i class="material-icons" style="font-size:16px;vertical-align:middle;">arrow_back</i> Back to List
                            </a>
                            <a href="tp-invoice-print.php?id=<?php echo htmlspecialchars($enc_id); ?>" class="btn btn-dark">
