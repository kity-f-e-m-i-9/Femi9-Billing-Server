<?php
/**
 * Renders the #divToPrint invoice markup shared by shop-invoice-print.php
 * (the logged-in Print page) and shop-invoice-pdf.php (the no-login PDF
 * endpoint used for WhatsApp sharing) — both need byte-identical HTML so
 * the shared PDF looks exactly like what the TP already sees on screen.
 *
 * All business logic (DB queries, GST computation, totals) stays in the
 * calling page, exactly as before — this function only takes the already-
 * computed values and returns markup. It intentionally has no dependency
 * on session/login state, so it's safe to call from the no-login endpoint.
 *
...
function render_shop_invoice_html(array $ctx, bool $forPdf = false): string {
    ...
    // invoice_items, Totalquantity123, TotalAMount123, Currency_symbol,
