<?php
/**
 * Renders the #divToPrint TP-invoice markup shared by
 * company/tp-invoice-print.php (the logged-in Print page) and
 * company/tp-invoice-pdf.php (the no-login PDF endpoint used for WhatsApp
 * sharing) — both need byte-identical HTML so the shared PDF looks exactly
 * like what the company user already sees on screen.
 *
 * All business logic (DB queries, GST computation, totals) stays in
 * TpInvoiceData.php — this function only takes the already-computed values
 * and returns markup. It intentionally has no dependency on session/login
 * state, so it's safe to call from the no-login endpoint.
 *
 * $forPdf switches off the print-specific @media rules that only make sense
 * inside a live browser tab — dompdf doesn't evaluate @media queries the way
 * a real browser does, and they'd just be dead weight in the PDF's own
 * <style> block.
 *
 * $show_carton_cols is passed separately (not derived from $ctx) because the
...
function render_tp_invoice_html(array $ctx, bool $show_carton_cols, bool $forPdf = false): string {
    ...
    // TotalCartons123, has_carton_data, courier_charges, discount_amount,
    // grand_total, has_gst_product, invoice_heading, result (amount-in-
    // words), TAXresult (tax-amount-in-words), Currency_symbol, Currency_Name.
...
/* dompdf renders a fixed A4 page with no viewport to shrink into, unlike a
   browser tab — the item table (with an extra carton column vs. the other
   invoice types) at the Print page's on-screen 14px/5px-padding sizing runs
   right up against (and on invoices with SGST/CGST rows, past) the page's
   usable width. Two changes fix this:
