 ...
 * @param string $userType User type (e.g., 'super_stockiest', 'stockiest')
...
function checkAndAwardDailyReward($db_conn, $userType, $userId, $invoiceId, $invoiceNumber) {
    ...
    try {
        ...
        // ✅ FIXED: Check correct table name - change to match your database
        ...
        if (!mysqli_stmt_execute($check_stmt)) {
            throw new Exception("Failed to execute check: " . mysqli_stmt_error($check_stmt));
        }
        ...
        // ✅ FIXED: Correct table name and parameter types
        ...
        if (!$insert_stmt) {
            throw new Exception("Failed to prepare insert statement: " . mysqli_error($db_conn));
        }
        
        // ✅ FIXED: Changed 'd' to 'i' for integer points
        mysqli_stmt_bind_param($insert_stmt, "sssiss", 
            $userType, $userId, $today, $points, $invoiceId, $invoiceNumber
        );
        
        if (!mysqli_stmt_execute($insert_stmt)) {
            throw new Exception("Failed to insert reward record: " . mysqli_stmt_error($insert_stmt));
        }
        
        $reward_id = mysqli_insert_id($db_conn);
        mysqli_stmt_close($insert_stmt);
        
        // Insert audit log (optional - skip if table doesn't exist)
        ...
        if ($audit_stmt) {
            mysqli_stmt_bind_param($audit_stmt, "sssiss", 
                $userType, $userId, $today, $points, $invoiceId, $invoiceNumber
            );
        ...
        // ✅ FIXED: Commit transaction properly
        if (!mysqli_commit($db_conn)) {
            throw new Exception("Failed to commit transaction: " . mysqli_error($db_conn));
        }
        ...
        // Log success
        error_log("✅ Daily Reward Success: User {$userType}:{$userId} awarded {$points} points for invoice {$invoiceNumber}");
        
        return $result;
        
    } catch (Exception $e) {
        // ✅ FIXED: Proper rollback check
        if (isset($db_conn) && $db_conn) {
            mysqli_rollback($db_conn);
        }
        
        $result['success'] = false;
        $result['message'] = 'Reward system error: ' . $e->getMessage();
        
        // Enhanced error logging
        error_log("❌ Daily Reward Error for User {$userType}:{$userId}: " . $e->getMessage());
