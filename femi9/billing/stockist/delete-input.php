...
// Fetch the input_stock_users record before deletion (prepared — no injection)
...
if ($record && (int)$record['product_id'] > 0) {
    ...
    try {
        ...
        // Reverse credit: input_qty ↓, closing_qty ↓ (floored at 0) — FOR UPDATE + ledger
        $stockService->reverseCredit(
            $product_id, $usertype, $userid, $input_qty,
            'st_input', $tempid, $createdBy,
