                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
									...
									<?php if(isset($_REQUEST['addesuccess'])){?><div class="alert alert-success">Customers added success.</div><?php }?>
								
								<?php if(isset($_REQUEST['alreadyexists'])){?><div class="alert alert-danger">Customer details already exists !</div>
									<?php }?>
...
<?php
$select_count_opstock13="select count(*) as numopstock12 from stock where user_id='$Login_user_IDvl'";
...
<form action="customer-user-invoice-action2.php" method="post" enctype="multipart/form-data">
...
<option value="<?=$CustomerID;?>" hidden=""><?=$inv_customer_name;?>, <?=$inv_customer_mobile;?></option>
<?php 
...
<?php }?>
    ...
    <div class="item">
	<select required="" name="pr_id" required="" style="width:100%;" class="prinput" autofocus onChange="showPrice(this.value)">
...
<input type="number" min="0" name="amount" id="amount" onKeyup="totalkm()" required="" placeholder="Price"></span>
		...
		<input type="number" min="0" name="total" id="output" readonly class="numberinput" required="" placeholder="Total">
...
<a href="customer-user-del-inv-product.php?inv_id=<?php echo $Invoice_ID_encode;?>&&rowid=<?php echo $ItemRowid;?>&&userid=<?=$CustomerID;?>&&actionremove"onclick="return confirm('You want to delete confirm?');"><span class="badge bg-danger">Remove</span></a>
...
function totalamount(){
    var roundtotal = <?php echo $TotalAMount123; ?>;
...
<form action="customer-user-invoice-submit" method="post" enctype="multipart/form-data" onSubmit="return confirm('Please make a confirm!');">
									...
									<?php }else{?>
									 ...
									 <div class="modal fade" id="composeModal" tabindex="-1" aria-labelledby="composeModalLabel" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-lg">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="composeModalLabel">Add Customer</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                </div>
...
<input type="text" name="gstin" class="form-control" placeholder="optional" maxlength="15" onkeypress="restrictGSTIN(event)">
</br>

<label class="form-label">Address</label>
<textarea name="address" class="form-control" onkeypress="restrictSpecialChars(event)" placeholder="optional"></textarea>
</br>

<!----<label class="form-label">Marketting Date*</label>
<input type="date" name="marketing_date" value="<?php echo date("Y-m-d");?>" required="" class="form-control">
</br>---->
...
<button type="submit" name="add-customer" class="btn btn-primary"><i class="material-icons">add</i>Add</button>
		...
		<form action="customer-user-invoice-action.php" method="post" enctype="multipart/form-data">
...
<tr>
<td align="left"><label class="form-label"><?=$lablenamedisplay;?>*</label></td>
<td align="right">
<button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#composeModal"><i class="material-icons">add</i>New</button>   
</td>
</tr>
</table>

<select required="" name="customer_id" class="form-control" autofocus style="margin-bottom:5px;">
<option value="" hidden="">Select</option>
<!----<option value="0">Walking Customer</option>---->
<?php 
$selectCusList="select * from ".$tablename." where user_id='$onboard_userID' order by name asc";
$fetch_Customers_list=mysqli_query($db_conn,$selectCusList);
while($result_Customers_list=mysqli_fetch_array($fetch_Customers_list))
		{
?>
...
<div class="item">
<select required="" name="pr_id" required="" style="width:100%;" onChange="showPrice(this.value)" class="prinput">
<option value="" hidden="">Select Product</option>
<?php $select_Products_list="select * from products order by id asc";
$fetch_Products_list=mysqli_query($db_conn,$select_Products_list);
while($result_Products_list=mysqli_fetch_array($fetch_Products_list))
