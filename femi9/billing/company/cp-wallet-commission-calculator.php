<?php
/**
 * CP Wallet Commission Calculator
 *
 * Admin-facing dry-run/execute/rollback tool for the monthly CP commission
 * wallet crediting that normally happens silently via
 * channel-partner/cron-cp-commission.php (a CLI-only cron meant to run at
 * 02:00 on the 1st of every month). If the cron never fires — wrong path,
 * missing log directory, server asleep, etc — no Channel Partner ever gets
 * credited and nothing shows it failed.
 *
 * This tool runs the exact same eligibility rule for a chosen month across
 * ALL channel partners, on demand, and shows per-CP detail: sales-sourced
 * commission vs deposit-floor commission, which basis won, who was skipped
 * because they were already credited (either by this tool earlier or by the
 * cron), and who did not qualify and why (no sales and no deposit).
 *
 ...
 * touching rows created by the cron. The cron's own dedup check (on
 * commission_type + user_id + from_date/to_date) is untouched by this tool,
 * so the two can't double-credit the same CP+month.
 *
...
$title         = "CP Wallet Commission Calculator";
...
function ensureCpWalletCommissionSchema(mysqli $dbConn): void
{
    ...
    if ($col && $col->num_rows === 0) {
        $dbConn->query("ALTER TABLE `wallet_monthly_sls_report`
            ADD COLUMN `execution_id` VARCHAR(50) NULL DEFAULT NULL
                COMMENT 'Set only for rows inserted via an admin wallet calculator tool; NULL means auto-credited by cron/dashboard trigger'
            AFTER `remarks`");
    }

    $dbConn->query("CREATE TABLE IF NOT EXISTS `wallet_cp_commission_execution_log` (
        ...
        `total_cps_processed` INT NOT NULL DEFAULT 0,
    ...
    COMMENT='Execution log for CP wallet monthly commission crediting with rollback tracking'");
}

ensureCpWalletCommissionSchema($dbConn);

// =====================================================================
// HELPER FUNCTIONS — mirrors channel-partner/cron-cp-commission.php exactly
// =====================================================================
...
function getAllChannelPartners(mysqli $dbConn): array
{
    $stmt = $dbConn->prepare("SELECT id, cp_id, name FROM channel_partners ORDER BY name ASC");
    if (!$stmt) { error_log("getAllChannelPartners prepare failed: " . $dbConn->error); return []; }
    $stmt->execute();
...
function getCpSales(mysqli $dbConn, string $cpId, string $from, string $to): array
{
    $stmt = $dbConn->prepare("SELECT COALESCE(SUM(total_amount),0) AS total FROM tp_invoices
        WHERE source_cp_id = ? AND invoice_date BETWEEN ? AND ?");
    $stmt->bind_param("sss", $cpId, $from, $to);
    $stmt->execute();
    $gross = (float)($stmt->get_result()->fetch_assoc()['total'] ?? 0);
    $stmt->close();

    $stmt = $dbConn->prepare("SELECT COALESCE(SUM(urs.total),0) AS total
    ...
    return ['gross' => $gross, 'returns' => $returns, 'net' => max(0, $gross - $returns)];
}

function getCpTotalDeposit(mysqli $dbConn, string $cpId): float
{
    $stmt = $dbConn->prepare("SELECT COALESCE(SUM(n.deposit_amount),0) AS total
    ...
    $stmt->execute();
    $deposit = (float)($stmt->get_result()->fetch_assoc()['total'] ?? 0);
    $stmt->close();
    return $deposit;
}

function findExistingCpCommissionCredit(mysqli $dbConn, string $cpId, string $from, string $to): ?array
{
    $stmt = $dbConn->prepare("SELECT commission_amount, execution_id FROM wallet_monthly_sls_report
        WHERE user_type = 'channel_partner' AND user_id = ?
          AND from_date = ? AND to_date = ? AND commission_type = 'CP Commission' LIMIT 1");
    $stmt->bind_param("sss", $cpId, $from, $to);
    $stmt->execute();
...
/**
 * Evaluate one CP's commission eligibility for the given month.
 * Mirrors channel-partner/cron-cp-commission.php's rule set exactly,
 * but never writes — it only reports what would happen / what already
 * happened.
 */
function evaluateCpCommission(mysqli $dbConn, array $cp, array $range): array
{
    $cpId = (string)$cp['id'];
    ...
    $result = [
        ...
        'reason'            => '',
        'existing_source'   => '', // 'this_tool' | 'auto_cron' — only for already_credited
    ];

    // 1. Already credited this CP+month? (dedup — either by this tool or the cron)
    $existing = findExistingCpCommissionCredit($dbConn, $cpId, $range['from'], $range['to']);
    if ($existing) {
        $result['status']            = 'already_credited';
        $result['commission_amount'] = (float)$existing['commission_amount'];
        $result['existing_source']   = empty($existing['execution_id']) ? 'auto_cron' : 'this_tool';
        $result['reason']            = $result['existing_source'] === 'auto_cron'
            ? 'Already credited automatically (monthly cron)'
            : 'Already credited by a previous run of this tool';
        return $result;
    }

    // 2. Sales sourced from this CP's stock, net of accepted returns
    $sales = getCpSales($dbConn, $cpId, $range['from'], $range['to']);
    $result['sales_gross']   = $sales['gross'];
    $result['sales_returns'] = $sales['returns'];
    $result['sales_net']     = $sales['net'];

    // 3. Total security deposit across this CP's assigned locations
    $result['deposit_total'] = getCpTotalDeposit($dbConn, $cpId);

    if ($result['sales_net'] <= 0 && $result['deposit_total'] <= 0) {
        $result['reason'] = 'No sourced sales and no location deposit found';
        return $result;
    }

    // 4. Commission = higher of 6% of sales or 2% of total deposit
    $fromSales   = round($result['sales_net'] * 0.06, 2);
    $fromDeposit = round($result['deposit_total'] * 0.02, 2);
    ...
    if ($commission <= 0) {
        $result['reason'] = 'Computed commission is zero';
        return $result;
    }

    $result['status']            = 'credited';
    $result['commission_pct']    = $pct;
    $result['basis']             = $basis;
    $result['commission_amount'] = $commission;
    $result['reason']            = $basis . ' — commission due';

    return $result;
}

function processCpWalletCommissionCalculation(mysqli $dbConn, string $monthYear, string $mode): array
{
    ...
    try {
        ...
        $executionId = 'WCPC-' . date('YmdHis') . '-' . substr(md5(uniqid((string)mt_rand(), true)), 0, 8);
        $results['execution_id'] = $executionId;

        $cps = getAllChannelPartners($dbConn);
        $details = [];
        foreach ($cps as $cp) {
            $details[] = evaluateCpCommission($dbConn, $cp, $range);
        }
        ...
        $results['summary'] = [
            'total_cps_processed'     => count($details),
        ...
        if ($mode === 'execute') {
            ...
            foreach ($details as $d) {
                ...
                $stmt->bind_param(
                    "sssssddsdsss",
                    $d['cp_internal_id'], $range['from'], $range['to'], $range['month'], $range['year'],
                    $d['sales_net'], $d['deposit_total'],
                    $d['cp_internal_id'], $d['commission_pct'],
                    $d['commission_amount'], $d['basis'], $executionId
                );

                if (!$stmt->execute()) {
                    throw new Exception("Insert failed for CP {$d['cp_code']}: " . $stmt->error);
                }
                $stmt->close();
            }

            $stmt = $dbConn->prepare("
                INSERT INTO wallet_cp_commission_execution_log
                (execution_id, execution_mode, month_year, total_cps_processed, total_credited,
                ...
                VALUES (?, 'execute', ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            if (!$stmt) throw new Exception("Prepare failed (wallet_cp_commission_execution_log): " . $dbConn->error);
            $stmt->bind_param(
                "ssiiiidsss",
                $executionId, $monthYear,
                $results['summary']['total_cps_processed'],
            ...
            $results['message'] = sprintf(
                'Execution complete. %d CP(s) credited (₹%s total), %d already credited, %d not eligible.',
                $totalCredited, inr_format($totalAmount, 2), $totalAlready, $totalNotEligible
            );
        } else {
            $results['message'] = sprintf(
                'Dry run complete. %d CP(s) would be credited (₹%s total), %d already credited, %d not eligible.',
                $totalCredited, inr_format($totalAmount, 2), $totalAlready, $totalNotEligible
            );
        }
        ...
        error_log("processCpWalletCommissionCalculation error: " . $e->getMessage());
...
function rollbackCpWalletCommissionExecution(mysqli $dbConn, string $executionId): array
{
    global $logged_user_id, $logged_user_type;
    $result = ['success' => false, 'message' => ''];

    try {
        $stmt = $dbConn->prepare("SELECT is_rolled_back FROM wallet_cp_commission_execution_log WHERE execution_id = ?");
        ...
        $stmt = $dbConn->prepare("DELETE FROM wallet_monthly_sls_report WHERE execution_id = ? AND commission_type = 'CP Commission'");
        $stmt->bind_param("s", $executionId);
        $stmt->execute();
        ...
        $stmt = $dbConn->prepare("
            UPDATE wallet_cp_commission_execution_log
        ...
        $result['message'] = sprintf(
            'Rollback successful. Removed %d wallet credit(s). CP wallet balances will reflect this on next view.',
            $deletedRows
        );
    } catch (Exception $e) {
        $dbConn->rollback();
        $result['message'] = 'Rollback failed: ' . $e->getMessage();
        error_log("rollbackCpWalletCommissionExecution error: " . $e->getMessage());
    }

    return $result;
}

function getAvailableCpWalletCommissionExecutions(mysqli $dbConn): array
{
    $stmt = $dbConn->prepare("
        SELECT execution_id, month_year, total_credited, total_commission_amount,
               executed_by_user_name, executed_at, is_rolled_back
        FROM wallet_cp_commission_execution_log
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if ($action === 'dry_run' || $action === 'execute') {
            ...
            if (empty($monthYear) || !preg_match('/^\d{4}-\d{2}$/', $monthYear)) {
                ...
                $actionResult = processCpWalletCommissionCalculation($dbConn, $monthYear, $action);
            }
            ...
            if (empty($executionId)) {
                ...
                $actionResult = rollbackCpWalletCommissionExecution($dbConn, $executionId);
            }
        }
    }
}

$availableExecutions = getAvailableCpWalletCommissionExecutions($dbConn);
...
<body>
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="page-header">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <h1 class="page-title">💰 CP Wallet Commission Calculator</h1>
                                    <p class="page-subtitle">Monthly Channel Partner commission wallet crediting — dry run, execute &amp; rollback</p>
                                </div>
                                <div>
                                    <span class="page-badge">Channel Partner</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-8">
                                <div class="card">
                                    <div class="card-header">
                                        ...
                                        Select Month for CP Commission Run
                                    </div>
                                    <div class="card-body">
                                        <form method="POST" id="calculationForm">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6 mb-4">
                                                    ...
                                                    <small class="text-muted">The sales month being evaluated (CP-sourced TP invoices, and current location deposits)</small>
                                                </div>
                                            </div>

                                            <div class="alert alert-info" role="alert">
                                                <strong>💡 How CP commission works:</strong><br>
                                                Each Channel Partner earns the higher of two figures for the selected month:
                                                <br>
                                                <code style="background:rgba(102,126,234,0.1);padding:4px 8px;border-radius:4px;color:#3730a3;">
                                                    Commission = MAX( 6% of TP-invoice sales sourced from this CP, 2% of total location deposit )
                                                </code>
                                                <br><br>
                                                Sales are net of accepted returns for the month. Deposit is the CP's current total security deposit
                                                across all assigned locations (not month-specific) — this acts as an earnings floor even in a
                                                zero-sales month.
                                                <br><br>
                                                <strong>⚠️ Why this tool exists:</strong> the normal crediting path is a CLI-only cron
                                                (<code>channel-partner/cron-cp-commission.php</code>) scheduled for 02:00 on the 1st of every month.
                                                If it never runs — wrong path, missing log folder, server down at that hour — no CP ever gets
                                                credited and nothing surfaces the failure. This tool lets you check and credit it manually, for any
                                                month, without waiting on the cron.
                                                <br><br>
                                                <strong>🔁 Safe to re-run:</strong> a CP already credited for a month (by this tool or the cron) shows
                                                as <em>Already Credited</em> and is skipped — no double payment.
                                            </div>
                                    ...
                                    <div class="card-body">
                                        <form method="POST" id="rollbackForm" onsubmit="return confirmRollback()">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <small class="text-muted">Rollback deletes only the wallet credits inserted by that run — cron-credited rows are untouched</small>
                                            </div>
                            ...
                            <div class="row mb-4">
                                <div class="col-md-3 col-sm-6 mb-3">
                                    <div class="stat-card primary">
                                        <div class="stat-label">Total CPs</div>
                                        <div class="stat-value"><?php echo (int)$summary['total_cps_processed']; ?></div>
                                ...
                                <div class="col-md-3 col-sm-6 mb-3">
                                    <div class="stat-card success">
                                        ...
                                        <small class="text-muted">Channel Partners getting paid</small>
                                    </div>
                                </div>
                                <div class="col-md-3 col-sm-6 mb-3">
                                                        ...
                                                        <?php foreach ($actionResult['details'] as $detail): ?>
                                                            ...
                                                            <tr class="<?php echo $rowClass; ?>" data-status="<?php echo htmlspecialchars($detail['status'], ENT_QUOTES, 'UTF-8'); ?>">
                                                                <td>
                                                                    ...
                                                                    <small class="text-muted"><?php echo htmlspecialchars($detail['cp_code'], ENT_QUOTES, 'UTF-8'); ?></small>
                                                                </td>
                                                                <td>
                                                                    ₹<?php echo inr_format((float)$detail['sales_net'], 0); ?>
                                                                    <?php if ((float)$detail['sales_returns'] > 0): ?>
                                                                        <br><small class="text-muted">(₹<?php echo inr_format((float)$detail['sales_gross'], 0); ?> − ₹<?php echo inr_format((float)$detail['sales_returns'], 0); ?> returns)</small>
                                                                    <?php endif; ?>
                                                                </td>
                                                                <td>₹<?php echo inr_format((float)$detail['deposit_total'], 0); ?></td>
                                                                <td>
                                                                    <?php if (!empty($detail['basis'])): ?>
                                                                        <span class="text-muted"><?php echo htmlspecialchars($detail['basis'], ENT_QUOTES, 'UTF-8'); ?></span>
                                                                    <?php else: ?>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    <?php elseif ($detail['status'] === 'already_credited'): ?>
                                                                        ...
                                                                        <br><small class="text-muted"><?php echo $detail['existing_source'] === 'auto_cron' ? 'via monthly cron' : 'via this tool'; ?></small>
                                                                    <?php else: ?>
    ...
    function submitForm(action) {
        if (action === 'execute') {
            if (!confirm('⚠️ Are you sure you want to EXECUTE and credit wallets?\n\nThis will:\n• Credit CP wallets for every Channel Partner eligible this month\n• Skip CPs already credited (no double payment)\n\nThis action can be rolled back later.')) {
                return;
            }
        }
    ...
    function confirmRollback() {
        ...
        return confirm('⚠️ Are you sure you want to ROLLBACK this execution?\n\nThis will permanently delete the wallet credits inserted by this run (cron-credited rows are not touched).\n\nThis action cannot be undone.\n\nContinue with rollback?');
    }
