...
<head>
    ...
    <!-- Title -->
    <title>Manage Top Performers : <?php echo $business_name;?></title>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="page-description">
									...
									<tr>
									<td>Manage Top Performers</td>
									<td><a href="performers_add" title="Add Offers">&#10011;</a></td>
									</tr>
									</table>
									</h1>
									
									
									<?php if(isset($_REQUEST['addedsuccess'])){?><div class="alert alert-success">Top Performers details added success.</div><?php }?>
								
								<?php if(isset($_REQUEST['updatedSuccess'])){?><div class="alert alert-info">Changes saved success.</div><?php }?>
								
								<?php if(isset($_REQUEST['deletedDone'])){?><div class="alert alert-warning">Deleted ! one Top Performers details deleted success.</div><?php }?>
...
<table id="datatable1" width="100%">
											...
											<tbody>
<?php
$select_product_list="select * from top_performar order by id desc";
...
?>
					...
					<?php if($result_product_list["particulars"]!=NULL){ echo $result_product_list["particulars"]; } else
						{ echo "-----";}?></td>
					...
					<td><?php echo date("d/m/Y",strtotime($result_product_list["posted_date"]));?></td>
					    ...
					    <div class="actions-group">
					        <a href="performers_edit?prid=<?=base64_encode($result_product_list['id']);?>" class="action-link" title="Edit"><i class="material-icons-outlined" style="font-size:17px;color:#667eea;">edit</i></a>
					        <a href="performers_del?prid=<?=base64_encode($result_product_list['id']);?>" class="action-link delete" title="Delete" onclick="return confirm('You want to delete confirm?');"><i class="material-icons-outlined" style="font-size:17px;color:#ef4444;">delete_outline</i></a>
					    </div>
					</td>
