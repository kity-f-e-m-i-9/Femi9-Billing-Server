<?php
// Bulk print for Manage OT Sales — prints every CONFIRMED invoice matching
// the current date range + category filter as one continuous document (one
// invoice per printed page), instead of opening each one individually via
// the per-row Print link. Reuses ot-sale-print.php's exact invoice markup/
// CSS, looped once per matching tempid.
...
$frdate = $_REQUEST['frdate'] ?? date('Y-m-d');
$todate = $_REQUEST['todate'] ?? date('Y-m-d');
$se_cat = trim($_REQUEST['se_cat'] ?? '');

// A checkbox selection from Manage OT Sales (particular orders picked by
// hand) always wins over the date/category filter — lets a company user
// print just the orders they actually ticked instead of everything in the
// filtered range. Falls back to the filter-based query when nothing was
// checked, so the plain "Print All Confirmed" click still works as before.
$selectedTempids = array_values(array_filter(array_map('strval', $_REQUEST['tempids'] ?? [])));

if (!empty($selectedTempids)) {
    $placeholders = implode(',', array_fill(0, count($selectedTempids), '?'));
    $sql = "SELECT tempid FROM ot_sales_invoice WHERE status = 'confirmed' AND tempid IN ($placeholders) ORDER BY id ASC";
    $stmt = $db_conn->prepare($sql);
    $stmt->bind_param(str_repeat('s', count($selectedTempids)), ...$selectedTempids);
    $stmt->execute();
    $tempids = array_column($stmt->get_result()->fetch_all(MYSQLI_ASSOC), 'tempid');
    $stmt->close();
} else {
    // ot_sales_invoice itself has no date column — the real order date lives
    // on ot_sales (its line items), same join ot-sale-view.php's own
    // date-filtered query already uses.
    $sql = "SELECT osi.tempid, MIN(os.date) AS order_date, osi.id
            FROM ot_sales_invoice osi
            JOIN ot_sales os ON os.tempid = osi.tempid
            WHERE osi.status = 'confirmed' AND os.date BETWEEN ? AND ?";
    $types = 'ss';
    ...
    $sql .= " GROUP BY osi.tempid, osi.id ORDER BY order_date ASC, osi.id ASC";
    $stmt = $db_conn->prepare($sql);
...
<head>
    ...
    <title>Invoices (<?= count($tempids) ?>) : <?php echo htmlspecialchars($business_name, ENT_QUOTES, 'UTF-8'); ?></title>
...
<div class="app align-content-stretch d-flex flex-wrap">
    ...
    <div class="app-container">
        ...
        <div class="app-content">
            ...
            <table align="right">
                <tr>
                    <td><button type="button" onclick="PrintDiv();" class="btn btn-dark m-b-xs m-r-xs">Print All (<?= count($tempids) ?>)</button></td>
            ...
            <?php if (empty($tempids)): ?>
                <div class="alert alert-warning">No confirmed invoices in this date range<?= $se_cat !== '' ? ' for category "' . htmlspecialchars($se_cat, ENT_QUOTES) . '"' : '' ?>.</div>
            <?php endif; ?>

            <div id="divToPrint">
<style type="text/css">
.maincontainar{width:100%;height:auto;border:1px solid #000;page-break-after:always;margin-bottom:20px;}
