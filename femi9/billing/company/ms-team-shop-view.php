...
function renderStatRow(array $row, array $byManager, array $rawStats, array $levelColorMap, int $rank = 0, int $indentLevel = 0, string $dateParam = ''): string {
    $id = (int)$row['id'];
    ...
    $html .= '<td><span class="stat-pill stat-shop">' . $sum['shops'] . '</span></td>';
    $html .= '<td><span class="stat-pill stat-got">' . $sum['got'] . '</span></td>';
    $html .= '<td><span class="stat-pill stat-no">' . $sum['no'] . '</span></td>';
    $html .= '<td>';
    if ($sum['shops'] > 0) {
        $html .= '<a class="btn-view-shop-list" target="_blank" rel="noopener" href="ms-team-shops.php?ms_ids=' . htmlspecialchars($idsParam) . htmlspecialchars($dateParam) . '">Shop List &rarr;</a>';
    } else {
...
function renderRowsRecursive(array $row, array $byManager, array $rawStats, array $levelColorMap, int $indentLevel, int $rank = 0, string $dateParam = ''): string {
    $html = renderStatRow($row, $byManager, $rawStats, $levelColorMap, $rank, $indentLevel, $dateParam);
    $kids = $byManager[(int)$row['id']] ?? [];
    foreach ($kids as $kid) {
        $html .= renderRowsRecursive($kid, $byManager, $rawStats, $levelColorMap, $indentLevel + 1, 0, $dateParam);
    }
    return $html;
}
?>
...
<head>
    ...
    <style>
        ...
        .stat-pill { font-size:11px; font-weight:600; padding:2px 9px; border-radius:6px; white-space:nowrap; }
        .stat-shop { background:#f0f1f2; color:#52525b; }
        .stat-got  { background:#e5f7e5; color:#0ca30c; }
        .stat-no   { background:#fbe6e6; color:#d03b3b; }
        .btn-view-shop-list {
            font-size:11px; font-weight:700; color:#2a78d6; background:#eaf2fc;
            padding:3px 11px; border-radius:6px; text-decoration:none; white-space:nowrap;
        }
        .btn-view-shop-list:hover { background:#d7e8fb; color:#2a78d6; }
            ...
            <div class="content-wrapper">
                <div class="container-fluid">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3 col-sm-6 mb-3">
                            <div class="kpi-card" style="--kpi-accent:#6b7280;--kpi-tint:#f0f1f2;">
                        ...
                        <div class="col-md-3 col-sm-6 mb-3">
                            <div class="kpi-card" style="--kpi-accent:#d03b3b;--kpi-tint:#fbe9e9;">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <table class="mt">
                                            ...
                                            <?php $rank = 0; foreach ($smRoots as $sm):
                                                $rank++;
                                                echo renderRowsRecursive($sm, $byManager, $rawStats, $levelColorMap, 0, $rank, $dateParam);
                                            endforeach; ?>
                                            </tbody>
                                        </table>
                                        </div>
