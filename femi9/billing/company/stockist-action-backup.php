...
$fetc_count_dist=mysqli_query($db_conn,$select_count_dist);
	...
	$valid_to = date ("Y-m-d", strtotime("+$valid_months days", strtotime($valid_from)));
		...
		'$merchantOrderId','$merchantTransactionId','$merchantUserId','$taluk_id','$DummySuperStockistID',
...
$data = array(
    ...
    'redirect_url' => ''.$CallbackURL_own_stockist.'?orderid='.$get_orderid.'',
...
//---------------------------------------------------------------------
		...
		echo "<script>window.location='stockist-add.php?distalready';</script>";
	}
	...
	include("stockist-coupon-method.php");
