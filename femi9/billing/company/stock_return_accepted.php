...
<head>
    ...
    <!-- Title -->
    <title>Stock Return Accepted : <?php echo $business_name;?></title>
...
?>
                                ...
                                <div class="card">
									...
									<div style="overflow-x:scroll;">
                                         <table id="datatable1" style="width:100%;">
											...
											<tbody>
					<?php $select_product_list="select * from user_return_stock where to_usertype='$Login_user_TYPEvl' and status='accept' and deleted_at is null order by id desc";
				...
				$totalqty=$result_qtytotal[0];}else{$totalqty="0";}
				
				//get damaged qty total
				$select_qtytotalDMGD="select sum(damaged_qty) from user_return_stock_items where returnid='$returnid'";
				$fetch_qtytotalDMGD=mysqli_query($db_conn,$select_qtytotalDMGD);
				$result_qtytotalDMGD=mysqli_fetch_array($fetch_qtytotalDMGD);
				if($result_qtytotalDMGD[0]!=NULL){
				$totalqtyDMNGED=$result_qtytotalDMGD[0];}else{$totalqtyDMNGED="0";}
				?>
  <tr valign="top">
                                                    <td><?php echo ++$i; ?></td>
...
<td><?=inr_format($totalamount, 2);?></td>						
													...
													<td>
			<a href="stock_return_details?returnid=<?=base64_encode($returnid);?>&&pagename=stock_return_accepted" data-bs-toggle="tooltip" data-bs-placement="top" title="View Details">
