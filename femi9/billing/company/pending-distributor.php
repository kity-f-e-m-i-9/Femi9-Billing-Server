...
<head>
    ...
    <!-- Title -->
    <title>Pending Distributor : <?php echo $business_name;?></title>
...
<table id="datatable1">
											...
											<tbody>
<?php $select_product_list="select * from distributor where account_status='pending' order by id asc";
...
else{$district_name=$result_product_list['district_id'];}
...
}else{$taluk_name=$result_product_list['taluk_id'];}

//
/*$pincode_id=$result_product_list['pincode_id'];
if (is_numeric($pincode_id)) {
...
}else{ $pincodeshow=$result_product_list['pincode_id'];}*/

$pincodeshow=$result_product_list['pincode_id'];
											
					$rowid=base64_encode($result_product_list["id"]);
					
					//onboard user Details
					if($result_product_list['onboard_userTYPE']=="candf")
					{
					$tblanem="c_and_f";	
					$lableshow_user="C&F";
					}
					else if($result_product_list['onboard_userTYPE']=="stockiest")
					{
					$tblanem="stockiest";	
					$lableshow_user="Stockist";
					}
					else
					{ 
					$tblanem="super_stockiest";
					$lableshow_user="Super Stockist";
					}
					
					$supersid=$result_product_list['onboard_userID'];
$select_ssdetails="select * from ".$tblanem." where temp_id='$supersid'";
...
<td>
			<a href="JavaScript:newPopup('Distributor-details?prid=<?php echo $rowid;?>&&actiondetails');" data-bs-toggle="tooltip" data-bs-placement="top" title="View Details">
	...
	popupWindow = window.open(
		url,'popUpWindow','height=450,width=750,left=350,top=200,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no,status=yes')
			    ...
			    <div class="actions-group">
			        <a href="delete_distributor_approval.php?distributorid=<?php echo $rowid;?>" class="action-link delete" title="Delete" onclick="return confirm('You want to delete confirm this Distributor (<?php echo ucwords($result_product_list["><i class="material-icons-outlined" style="font-size:17px;color:#ef4444;">delete_outline</i></a>
			    </div>
			</td>
