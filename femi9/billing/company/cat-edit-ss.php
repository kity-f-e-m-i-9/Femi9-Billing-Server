...
$title="Update Category (Super Stockist)";
$manage_url="cat-view-ss";
$manage_title="Manage Category (Super Stockist)";
$message_title="Category (Super Stockist)";

// FIXED - URL decode first, then base64 decode, then cast to int
$prid = $_REQUEST['prid'] ?? '';
$prid = base64_decode(urldecode($prid));
$prid = (int)$prid; // always an integer ID - prevents SQL injection
...
$stmt = mysqli_prepare($db_conn, "SELECT * FROM super_stockiest_category WHERE id = ?");
mysqli_stmt_bind_param($stmt, "i", $prid);
mysqli_stmt_execute($stmt);
$result_product_list = mysqli_fetch_array(mysqli_stmt_get_result($stmt));
...
<input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token'], ENT_QUOTES, 'UTF-8'); ?>">
                                        ...
                                        <div class="example-container">
                                            <div class="example-content">
                                                ...
                                                <input type="number" min="0" onkeypress="restrictnumber(event)" class="form-control" name="target_amount" value="<?=$result_product_list['target_amount'];?>" required="required" max="999999" placeholder="Target Amount(Rs.)"/>
												<br/>
												...
												<button type="submit" name="UpdateSuperStockistCategory" class="btn btn-primary">Update</button>
