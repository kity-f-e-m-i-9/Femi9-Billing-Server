...
if (isset($_POST['add-record'])) {
    ...
    $stmt = $db_conn->prepare(
        "INSERT INTO neksomo_llp_piece_purchase_rates (product_id, effective_date, rate_per_piece, gst_rate, gst_type, created_by)
         VALUES (?, ?, ?, ?, ?, ?)"
    );
    ...
    redirectWithMessage('neksomo-llp-piece-purchase-rate.php', "addesuccess&count=$added&skipped=$skipped");
}
...
if (isset($_POST['update-record'])) {
    ...
    $stmt = $db_conn->prepare(
        "UPDATE neksomo_llp_piece_purchase_rates SET product_id=?, effective_date=?, rate_per_piece=?, gst_rate=?, gst_type=? WHERE id=?"
    );
    ...
    redirectWithMessage('neksomo-llp-piece-purchase-rate-manage.php', 'updatedSuccess');
}
