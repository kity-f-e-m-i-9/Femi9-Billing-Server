...
if (!isset($_POST['csrf_token']) || !isset($_SESSION['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
    redirectWithMessage('neksomo-llp-piece-purchase-rate.php', 'error');
}
...
if (isset($_POST['add-record'])) {
    ...
    $stmt = $db_conn->prepare(
        "INSERT INTO neksomo_llp_piece_purchase_rates (product_id, effective_date, rate_per_piece, created_by)
         VALUES (?, ?, ?, ?)"
    );
    ...
    if ($dupe) {
        redirectWithMessage('neksomo-llp-piece-purchase-rate.php', 'duplicate');
    }
    redirectWithMessage('neksomo-llp-piece-purchase-rate.php', $ok ? 'addesuccess' : 'error');
}
...
if (isset($_POST['update-record'])) {
    ...
    if (!$update_id || !$product_id || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $effective_date) || $rate_per_piece === false || $rate_per_piece < 0) {
        redirectWithMessage('neksomo-llp-piece-purchase-rate-manage.php', 'error');
    }

    $stmt = $db_conn->prepare(
        "UPDATE neksomo_llp_piece_purchase_rates SET product_id=?, effective_date=?, rate_per_piece=? WHERE id=?"
    );
    ...
    if ($dupe) {
        redirectWithMessage('neksomo-llp-piece-purchase-rate-manage.php', 'duplicate');
    }
    redirectWithMessage('neksomo-llp-piece-purchase-rate-manage.php', $ok ? 'updatedSuccess' : 'error');
}

redirectWithMessage('neksomo-llp-piece-purchase-rate.php');
