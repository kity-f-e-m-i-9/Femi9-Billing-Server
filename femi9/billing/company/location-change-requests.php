...
// Oversight of every DM's shop-location-recapture-limit request, regardless
// of which Sales BDM it's routed to (BDM only sees requests in their own
// assigned districts — see salesbdm/location-change-requests.php) — Company
// can see the full history across every district, who requested it, who
// accepted/rejected it and why, and can also review a pending request
// directly (e.g. if the assigned BDM is slow to act).
...
$pendingCount = (int)($db_conn->query("SELECT COUNT(*) c FROM ms_shop_location_requests WHERE status = 'pending'")->fetch_assoc()['c'] ?? 0);
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .lcr-filter { background:#fff; border:1px solid rgba(11,11,11,0.10); border-radius:10px; padding:14px 18px; margin-bottom:20px; }
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="page-description">
                                    ...
                                    <p class="text-muted" style="font-size:13px;">Every DM's shop-location-unlock request across all districts — who asked, who accepted/rejected it, and why.</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="card-body" style="overflow-x:auto;">
                                <table class="mt">
                                    ...
                                    <?php if (empty($requests)): ?>
                                        <tr><td colspan="8" class="text-center text-muted" style="padding:24px;">No requests found for this filter.</td></tr>
                                    <?php else: foreach ($requests as $r): ?>
                                        <tr id="lcrRow<?php echo $r['id']; ?>">
                                            ...
                                            <td><?php echo htmlspecialchars($r['shop_district'] ?? '—'); ?></td>
