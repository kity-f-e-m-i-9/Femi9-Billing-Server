...
if (isset($_REQUEST['delreceiptact'])) {
    ...
    if ($receipt_to_delete) {
        ...
        // ── Reverse stock only when this is the LAST stock-type receipt ─────────
        //
        // courier_charge receipts never touch stock — skip them entirely.
        //
        // For advance_product / regular receipts we only reverse stock when no
        // other stock-type receipts will remain after this deletion.
        //
        // Why: distributors and shops can now have multiple 'regular' receipts
        // (one from submit + additional payment-tracking receipts added later).
        // Deleting a payment-tracking receipt should NOT reverse stock — only
        // deleting the very last receipt signals "I want to edit this invoice."
        //
        // SS/Stockist can also have multiple advance receipts for partial
        // payments.  Same logic applies.
        if (in_array($del_payment_type, ['advance_product', 'regular'], true)) {

            // Count stock-type receipts that will remain AFTER this one is deleted
            $chk = $db_conn->prepare(
            ...
            $chk->bind_param('si', $invid, $rcptid);
            $chk->execute();
            [$remaining_stock_receipts] = $chk->get_result()->fetch_row();
            $chk->close();

            if ((int)$remaining_stock_receipts === 0) {
                // Last stock-type receipt → reverse stock so invoice can be edited
                ...
                if (!$reversalResult['success']) {
                    ...
                    error_log(
                        "RECEIPT DELETE: Stock reversed for inv=$invid (last receipt) — " .
                        $reversalResult['items_reversed'] . " entry/entries"
                    );
                }
            } else {
                // Other stock-type receipts remain — this is a payment correction,
                // do NOT reverse stock
                error_log(
                    "RECEIPT DELETE: Skipping stock reversal for inv=$invid — " .
                    "$remaining_stock_receipts other stock receipt(s) remain"
                );
            }
        }
...
if (isset($_POST['addreceipt'])) {
    ...
    $post_payment_type = $_POST['payment_type']  ?? '';  // 'invoice'|'courier'|'payment'
    $received_amount   = round(floatval($_POST['receivedamount'] ?? 0), 2);
    ...
    // ── Running balance: recalculate pending fresh from DB ────────────────────
    // Never trust the form's receivableamount — always derive from live data.
    // This prevents overpayment even if the form sends a stale/tampered value.
    //
    // pending_product = invoice_total_excl_courier - SUM(received WHERE type IN advance/regular)
    // pending_courier = courier_charges             - SUM(received WHERE type = courier_charge)
    $stmt_pending = $db_conn->prepare(
        "SELECT
            SUM(CASE WHEN payment_type IN ('advance_product','regular') THEN received ELSE 0 END) AS paid_product,
            SUM(CASE WHEN payment_type = 'courier_charge'               THEN received ELSE 0 END) AS paid_courier
         FROM receipt WHERE inv_id = ?"
    );
    $stmt_pending->bind_param('s', $post_invid);
    $stmt_pending->execute();
    $paid_row        = $stmt_pending->get_result()->fetch_assoc();
    $stmt_pending->close();

    $paid_product    = round(floatval($paid_row['paid_product'] ?? 0), 2);
    $paid_courier    = round(floatval($paid_row['paid_courier'] ?? 0), 2);
    $pending_product = round(max(0.0, $invoice_amount_only - $paid_product), 2);
    $pending_courier = round(max(0.0, $courier_charges     - $paid_courier), 2);

    // Determine the server-side pending limit for THIS payment type
    $server_pending = match($post_payment_type) {
        'invoice' => $pending_product,   // SS/Stockist advance
        'payment' => $pending_product,   // Distributor/Shop/Customer product payment
        'courier' => $pending_courier,   // Courier charges (all types)
        default   => 0.0,
    };

    // Guard: reject if nothing pending or payment exceeds what's outstanding
    if ($received_amount <= 0 || $received_amount > $server_pending + 0.01) {
        echo "<script>window.location='$redirectBase&InvalidAmount';</script>";
        exit;
    }

    // Clamp to server-pending (handles floating-point rounding)
    $received_amount = min($received_amount, $server_pending);

    // Running balance stored in this receipt row:
    //   invoice_amount = how much was pending BEFORE this payment (the "due" at this point)
    //   received       = how much was paid in THIS transaction
    //   receivable     = balance still outstanding AFTER this payment
    $balance_amount = round($server_pending - $received_amount, 2);
    ...
    if ((int)$dup_count === 0) {
        ...
        if ($is_advance_mandatory) {
            ...
            // ── Distributor / Super Distributor / Shop / Customer ─────────────
            // Stock already moved at submit — only payment tracking here.
            if ($post_payment_type === 'payment') {
                $db_payment_type = 'regular';
                $receipt_method  = mysqli_real_escape_string($db_conn, $_POST['receipt_method']  ?? '');
                $receipt_remarks = mysqli_real_escape_string($db_conn, htmlspecialchars($_POST['receipt_remarks'] ?? '', ENT_QUOTES));
        ...
        // Insert receipt with correct running balance values:
        //   invoice_amount = pending before this payment (what was "due" at this moment)
        //   received       = paid now
        //   receivable     = balance still owed after this payment
        $ins = $db_conn->prepare(
            "INSERT INTO receipt
        ...
        $ins->bind_param(
            ...
            $post_receiptid,
            $post_invid,
            $server_pending,      // invoice_amount = pending at time of this payment
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="femi-table">
                                        ...
                                        <?php foreach ($all_receipts as $i => $r):
                                            ...
                                            $balance_after = floatval($r['receivable'] ?? 0);
                                            $balance_color = $balance_after < 0.01 ? '#065f46' : '#b45309';
                                    ...
                                    <?php if (!$needs_invoice_payment && !$needs_courier_payment): ?>
                                    <div class="alert alert-success" style="margin-top:20px">
                                        <i class="material-icons">check_circle</i>
                                        <div><strong>Invoice fully paid.</strong> All amounts have been received.</div>
                                    </div>
                                    ...
                                    <?php if ($needs_invoice_payment): ?>
                                    <!-- Form B1: Product amount (partial payment tracking) -->
                                    <div class="payment-card combined">
                                        <h4>
                                        ...
                                        <form method="POST" onsubmit="return confirm('Submit payment receipt?')">
                                            ...
                                            <input type="hidden" name="receiptid"        value="<?= e(GeraHash(10) . '/RCPT/' . $temp_date . '/' . $temp_time . 'P') ?>">
                                            <input type="hidden" name="receivableamount" value="<?= $invoice_amount_pending ?>">
                                            ...
                                            <input type="number" class="form-control" value="<?= $invoice_amount_pending ?>" disabled>

                                            <label class="form-label">Received Amount</label>
                                            <input type="number" id="pay_received" name="receivedamount" required
                                                   min="0.01" max="<?= $invoice_amount_pending ?>" step="0.01"
                                                   class="form-control" placeholder="Enter amount received"
                                                   oninput="calcBalance('pay_received','<?= $invoice_amount_pending ?>','pay_balance')">
                                            ...
                                            <label class="form-label">Remaining After This</label>
                                            <input type="number" id="pay_balance" class="form-control" readonly>

                                            <button type="submit" name="addreceipt" class="btn-submit teal" style="margin-top:6px">
                                                <i class="material-icons">add</i> Submit Payment Receipt
                                            </button>
                                        </form>
                                    </div>
                                    <?php else: ?>
                                    <div class="paid-pill" style="margin-bottom:14px">✅ Product amount fully received</div>
                                    <?php endif; ?>

                                    <?php if ($needs_courier_payment): ?>
                                    <!-- Form B2: Courier charges -->
                                    <div class="payment-card courier">
                                        <h4>
                                            <i class="material-icons">local_shipping</i>
                                            Pay Courier Charges
                                        ...
                                        <form method="POST" onsubmit="return confirm('Submit courier charge receipt?')">
                                            ...
                                            <input type="hidden" name="invuser"          value="<?= e($getinvuser) ?>">
                                            <input type="hidden" name="payment_type"     value="courier">
                                            <input type="hidden" name="receiptid"        value="<?= e(GeraHash(10) . '/RCPT/' . $temp_date . '/' . $temp_time . 'BC') ?>">
                                            <input type="hidden" name="receivableamount" value="<?= $courier_amount_pending ?>">
                                            ...
                                            <label class="form-label">Received Amount</label>
                                            <input type="number" id="cc2_received" name="receivedamount" required
                                                   min="0.01" max="<?= $courier_amount_pending ?>" step="0.01"
                                                   class="form-control" placeholder="Enter amount received"
                                                   oninput="calcBalance('cc2_received','<?= $courier_amount_pending ?>','cc2_balance')">
                                            ...
                                            <label class="form-label">Remaining After This</label>
                                            <input type="number" id="cc2_balance" class="form-control" readonly>
