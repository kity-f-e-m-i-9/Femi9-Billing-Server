...
$fetc_count_dist=mysqli_query($db_conn,$select_count_dist);
	...
	$useridtext="FEMI9-R-".$format_num."";
        ...
        $sql="insert into shop (state_id,temp_id,user_icon,name,district_id,email,mobile_number,username,
		password,plan_amount,
		valid_months,valid_from,valid_to,amount_method,amount_status,ref_number,
		...
		'$shop_cat','$country_code','$landline')";
		mysqli_query($db_conn,$sql);
		
		echo "<script>window.location='Shop-manage?addedsuccess';</script>";


}else{
		//this districtwise super stockiest already exists.
		echo "<script>window.location='Shop-add?distalready';</script>";
	}
	
	}else{echo "<script>window.location='Shop-add?invalidparameters';</script>";}
