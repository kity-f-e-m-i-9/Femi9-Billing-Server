...
<head>
    ...
    <!-- Title -->
    <title>Manage Sales BDM : <?php echo $business_name;?></title>
                        ...
                        <div class="row">
                            <div class="col">
                                <div class="page-description">
									...
									<?php if(isset($_REQUEST['addesuccess'])){?><div class="alert alert-success">Sales BDM added success.</div><?php }?>

									<?php if(isset($_REQUEST['updatedSuccess'])){?><div class="alert alert-info">Changes saved success.</div><?php }?>

									<?php if(isset($_REQUEST['deletedDone'])){?><div class="alert alert-warning">Deleted ! one Sales BDM details deleted success.</div><?php }?>

                                    <h1>
									<table class="headertble">
									<tr>
									<td>Manage Sales BDM</td>
									<td><a href="salesbdm_add.php" title="Add Sales BDM">&#10011;</a></td>
									</tr>
									</table>
									</h1>
                                </div>
                            ...
                            <div class="col">
                                <div class="card">
                                    <div class="card-body">
										...
										$teamLevelMap = [];
										$resTeamLevels = $db_conn->query("SELECT id, level_name FROM salesbdm_team_levels");
										if ($resTeamLevels) {
											while ($rowTeamLevel = $resTeamLevels->fetch_assoc()) {
												$teamLevelMap[(int)$rowTeamLevel['id']] = $rowTeamLevel['level_name'];
											}
										}
										$select_product_list="select * from sales_bdm_staff order by id desc";
										...
										while($result_product_list=mysqli_fetch_array($fetch_product_list))
										{
											...
											?>
                                                ...
                                                <tr>
                                                    <td><?php echo ++$i; ?></td>
                                                    <td><?php echo $result_product_list["bdm_name"];?></td>
													<td><?php echo $result_product_list["country_code"];?>&nbsp;<?php echo $result_product_list["bdm_mobile"];?></td>
													<td>
                                                        <?php
                                                        ...
                                                        <div class="password-container">
                                                            ...
                                                            <button onclick="copyPassword('<?=htmlspecialchars($storedPassword, ENT_QUOTES)?>', <?=$rowIdUnique?>)" class="copy-btn" title="Copy password">
                                                                <i class="material-icons-outlined">content_copy</i>
                                                            </button>
                                                            <input type="hidden" id="pwd-value-<?=$rowIdUnique?>" value="<?=htmlspecialchars($storedPassword, ENT_QUOTES)?>">
                                                        </div>
                                                        </td>
													<td><?php echo $result_product_list["bdm_email"];?></td>
													<td><?php echo $result_product_list["bdm_address"];?></td>

													<td>
													<?php
													$bdmTeamLevelId = (int)($result_product_list['team_level_id'] ?? 0);
													if ($bdmTeamLevelId && isset($teamLevelMap[$bdmTeamLevelId])) {
														echo htmlspecialchars($teamLevelMap[$bdmTeamLevelId]);
													} else {
													...
													<td>
													<?php
													$bdmTarget = $result_product_list['monthly_target_amount'];
													echo ($bdmTarget !== null && $bdmTarget !== '') ? ('&#8377;' . number_format((float)$bdmTarget, 2)) : '<span class="text-muted">-</span>';
													?>
													</td>
