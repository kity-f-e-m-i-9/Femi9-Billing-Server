...
if ($warehouseId !== null) {
    ...
    // check at submit time (tp-invoice-action.php). Showing the combined
    // total here let a user pick a warehouse with little/no stock while the
    ...
    ");
    $stmt->bind_param("si", $uid, $warehouseId);
} else {
    ...
    // GROUP BY p.id, summing across any per-warehouse rows. HAVING (not
    // WHERE) filters the summed total. p.stockist_price/p.mrp are
    // functionally dependent on p.id, safe to select alongside the GROUP BY.
    $stmt = $db_conn->prepare("
