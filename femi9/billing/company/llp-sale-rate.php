...
$db_conn->query("
    CREATE TABLE IF NOT EXISTS femi9_llp_sale_rates (
...
$products = $db_conn->query("SELECT id, productName, pieces_per_pack, unit_type, gst, gst_type FROM products WHERE deleted_at IS NULL AND (temp_id IS NULL OR temp_id NOT LIKE 'NKS-%') ORDER BY productName ASC")->fetch_all(MYSQLI_ASSOC);
?>
...
<body>
    ...
    (function ($) {
        ...
        $('#productSelect').on('change', function () {
            ...
            $('#rateLabelText').text(isPack ? 'Rate/Pack (₹)' : 'Rate/Piece (₹)');
            ...
            if ($opt.val()) {
                $('#gstHint').text(
                    'GST ' + gstRate + '% ' + gstType + ' — ' +
                    (gstType === 'inclusive'
                        ? 'this rate already has GST built in; Gross Profit uses the pre-tax portion.'
                        : 'this rate is already pre-tax and used as-is for Gross Profit.')
                );
            } else {
                $('#gstHint').text('');
            }
