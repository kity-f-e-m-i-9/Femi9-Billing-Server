...
<div class="row">
                                            <div class="table-responsive">
                                                <table class="table">
                                                  ...
                                                  <?php if($amount_received_fully==0){?><th scope="col"></th><?php }?>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
													<?php
...
xmlhttp.send();}
			...
			<script>
			// Auto-suggest next invoice number (CFSH/FY/LLP{n} napkin /
			// CFSHD/FY/LLP{n} diaper). Suggested on page load (napkin default,
...
<label class="form-label"><?=$lablenamedisplay;?>*</label>
<select required="" name="customer_id" class="js-states form-control" tabindex="-1" style="display: none;width:100%;" autofocus>
<option value="" hidden="">Select</option>
<?php 
$selectCusList="select * from ".$tablename." where onboard_userTYPE='$onboard_userTYPE' and onboard_userID='$onboard_userID' order by name asc";
$fetch_Customers_list=mysqli_query($db_conn,$selectCusList);
...
<?php if($amount_received_fully==0){?>
    <div class="item">
	<select required="" name="pr_id" required="" style="width:100%;" onChange="showPrice(this.value); invNumberSuggest(this.value);" class="prinput">
<option value="" hidden="">Select Product</option>
<?php $select_Products_list="select * from products order by id asc";
$fetch_Products_list=mysqli_query($db_conn,$select_Products_list);
while($result_Products_list=mysqli_fetch_array($fetch_Products_list))
