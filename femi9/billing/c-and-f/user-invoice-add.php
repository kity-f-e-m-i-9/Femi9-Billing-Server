                        ...
                        <div class="row">
                            <div class="col-md-12">
								...
								<?php if(isset($_REQUEST['invoicealready'])){?><div class="alert alert-danger">Invoice Number already exists!</div>
									...
									<tr>
									<td>
									<?php if($_REQUEST['action']=="edit"){ echo "Update > ";} ;?>
									<?=$displaytitle;?></td>
...
{
$selectCusList="select * from ".$tablename." where account_status='active' and onboard_userTYPE='$onboard_userTYPE' and onboard_userID='$onboard_userID' order by name asc";
...
<?php if($amount_received_fully==0){?>
    ...
    <div class="item">
	<select required="" name="pr_id" required="" class="prinput" autofocus onChange="showPrice(this.value)" style="width:100%;">
...
<input type="number" min="0" name="amount"  step="any" id="amount" onKeyup="totalkm()" required="" placeholder="Price"></span>
		...
		<input type="number" min="0" name="total" step="any" id="output" class="numberinput" required="" placeholder="Total">
...
<?php }?>
	...
	<?php }?>
                   ...
                   <p>Invoice Number: <?php if($get_action=="edit") {?>
...
<?php $_SESSION['INVOICEFINISH']="1";?>
            ...
            <div class="bold">Subtotal <span>
			<input type="number" min="0" class="form-control" step="any" value="<?=$TotalAMount123;?>" id="subtotal" disabled></span>
			</div>
...
mysqli_query($db_conn,$update_zero_invoice);		
		...
		<!---- <p class="bold">Credit <span>
		  <input type="number" readonly step="any" value="<?=$creditless;?>" id="credit" min="0" name="credit" required="">
		 </span></p>
		<br/>---->
		
		<input type="hidden" name="roundoff" value="<?=number_format($roundoff,2,'.','');?>"/>
		
		<div class="bold">Round off <span>
		 <input type="number" min="0" step="any" value="<?=number_format($roundoff,2,'.','');?>" disabled class="form-control">
		 </span>
		 </div>
		 
		 <div class="bold">Courier Charges <span>
...
<div class="bold">Invoice Date <span>
...
<?php }else{?>
		<div class="bold">Received Amount<span>
		 <input type="number" min="0" required="" step="any" max="<?=number_format($roundvalue,2,'.','');?>" id="receivedamount" onkeyup="receiptamount()" name="receivedamount" class="form-control">
		 </span>
		 </div>
...
<label class="form-label"><?=$lablenamedisplay;?>*</label>
<select required="" name="customer_id" class="form-control" autofocus onchange="showstockavailable(this.value)">
<option value="" hidden="">Select</option>
<?php 
$selectCusList="select * from ".$tablename." where account_status='active' and onboard_userTYPE='$onboard_userTYPE' and onboard_userID='$onboard_userID' order by name asc";
$fetch_Customers_list=mysqli_query($db_conn,$selectCusList);
