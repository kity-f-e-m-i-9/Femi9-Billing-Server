...
if (!empty($_REQUEST['update_invoice_date'])) {
    ...
    mysqli_query($db_conn, "UPDATE user_invoice SET date='$upd_date' WHERE inv_id='$invoice_id' AND from_user_type='$Login_user_TYPEvl' AND from_user_id='$Login_user_IDvl'");
    mysqli_query($db_conn, "UPDATE user_invoice_items SET date='$upd_date' WHERE inv_id='$invoice_id' AND from_user_type='$Login_user_TYPEvl' AND from_user_id='$Login_user_IDvl'");
}
...
$stmtItems = $db_conn->prepare("SELECT pr_id, qty FROM user_invoice_items WHERE inv_id=? AND from_user_type=? AND from_user_id=?");
...
foreach ($items as $item) {
    ...
    $stmtL = $db_conn->prepare("INSERT INTO territory_partner_stock_ledger
        ...
        VALUES (?, ?, 'deduct', ?, ?, ?, 'tp_invoice', ?, 'shop invoice', ?)");
    $stmtL->bind_param('iiiiiss', $tp_id, $prid, $qty, $before, $after, $invoice_id, $created_by);
    $stmtL->execute(); $stmtL->close();
}

// Update invoice totals
mysqli_query($db_conn, "UPDATE user_invoice SET sub_total='$SubTotal',discount='$discount',total='$total_amount',roundoff='$roundoff',courier_charges='$courier' WHERE inv_id='$invoice_id' AND from_user_type='$Login_user_TYPEvl' AND from_user_id='$Login_user_IDvl'");
...
$usertype        = $inv['to_user_type'] ?? 'shop';
$userid          = $inv['to_user_id']   ?? '';
...
if ($existingReceipt) {
    ...
    mysqli_query($db_conn, "INSERT INTO receipt (receiptid,inv_id,invoice_amount,received,receivable,date,from_user_type,from_user_id,to_user_type,to_user_id,receipt_method,receipt_remarks) VALUES ('$invoice_id','$invoice_id','$total_amount','$receivedamount','$receivable','$receiptdate','$from_utype','$from_uid','$usertype','$userid','$receipt_method','$receipt_remarks')");
}

unset($_SESSION['ACTIONEDIT']);
echo "<script>window.location='shop-invoice-print.php?invoiceid=".base64_encode($invoice_id)."';</script>";
