...
// For Super Stockist: No company dropdown needed
// Payer type is fixed to 'stockiest' (stockists under this super stockist)
$is_super_stockist = ($logged_user_type === 'super_stockiest');

// Get company profiles only for non-Super Stockist users
$company_profiles = [];
if (!$is_super_stockist) {
...
<body>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <form action="advance-payment-action.php" method="POST" id="advancePaymentForm" onsubmit="return validateForm();">
                                            ...
                                            <div class="row">
                                                ...
                                                <?php if (!$is_super_stockist): ?>
                                                <!-- Company Profile (Only for Company login) -->
                                                ...
                                                <div class="col-md-6 mb-3">
                                                    ...
                                                    <?php if ($is_super_stockist): ?>
                                                        <!-- For Super Stockist: Payer type is fixed to Stockist -->
                                                        <input type="text" class="form-control" value="Stockist" readonly>
                                                        <input type="hidden" name="from_user_type" id="from_user_type" value="stockiest">
                                                        <small class="text-muted">Only stockists under you can make payments</small>
                                                    <?php else: ?>
                                                        <!-- For Company login: Can select Super Stockist or Stockist -->
    ...
    <script>
    const isSuperStockist = <?php echo $is_super_stockist ? 'true' : 'false'; ?>;
    ...
    function loadUsers() {
        ...
        // For company login, require company selection
        if (!isSuperStockist) {
        ...
        // Add company_id only for non-Super Stockist
        if (!isSuperStockist) {
            ajaxData.company_id = document.getElementById('company_id').value;
        }
    ...
    /**
     * Reload users when company changes (only for company login)
     */
    if (!isSuperStockist) {
    ...
    /**
     * For Super Stockist: Auto-load stockists on page load
     */
    if (isSuperStockist) {
        window.addEventListener('DOMContentLoaded', function() {
            // Auto-load stockists for super stockist
            loadUsers();
        });
    }
