...
?>
						...
						if($ResultRecords12['to_usertype']=="territory_partner") {
							$stmt_tp_disp = $db_conn->prepare("SELECT name, mobile FROM territory_partners WHERE id=?");
							$tp_disp_id = (int)$send_to;
							$stmt_tp_disp->bind_param("i", $tp_disp_id);
							$stmt_tp_disp->execute();
							$result_godowndetails2 = $stmt_tp_disp->get_result()->fetch_assoc();
							$stmt_tp_disp->close();
							if (!$result_godowndetails2) $result_godowndetails2 = ['name'=>'Unknown TP', 'mobile'=>''];
							// map to expected keys
							$result_godowndetails2['mobile_number'] = $result_godowndetails2['mobile'];
						} else {
