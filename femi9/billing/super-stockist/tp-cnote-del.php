...
// Only delete if still pending — scoped to CNs against THIS SS's own issued
// invoices only.
$s = $db_conn->prepare("SELECT status FROM user_return_stock WHERE returnid=? AND from_usertype='territory_partner' AND to_usertype='super_stockiest' AND to_userid=? LIMIT 1");
