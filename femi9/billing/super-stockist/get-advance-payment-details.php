<?php
/**
 * Get Advance Payment Details - Modal Content (Super Stockist)
 * Femi9 Billing Application
 *
 * Mirrors company/get-advance-payment-details.php, scoped so a super
 * stockist can only view payments where they are the receiver (matching
 * the same scoping already used by get-advance-payments-data.php's listing
 * query) — otherwise any logged-in super stockist could view any payment
 * system-wide just by guessing/incrementing the id.
 */
...
// Fetch payment details — only if this super stockist is the receiver
...
$stmt->bind_param("is", $payment_id, $logged_user_id);
$stmt->execute();
