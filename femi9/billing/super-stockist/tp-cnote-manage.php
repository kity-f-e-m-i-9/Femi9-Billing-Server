...
// The SS identifier used throughout this feature — matches
// territory_partners.onboard_ss_id's format (a long temp_id string, not the
// numeric `id`) and the `stock` table's user_id for user_type='super_stockiest'.
// tp_invoices.created_by_user_id is NOT used for scoping: it's blank on a lot
// of real invoices (only backfilled for one old invoice-number pattern), so
// ownership is derived from the TP's own onboard_ss_id instead — reliable
// since an SS can only ever invoice a TP they themselves onboarded.
$ss_key = (string)$Login_user_IDvl;

// Load TP credit notes — only against invoices THIS SS issued.
$s = $db_conn->prepare("
    ...
    WHERE urs.from_usertype = 'territory_partner' AND urs.to_usertype = 'super_stockiest' AND urs.to_userid = ?
      AND tpi.created_by_user_type = 'super_stockiest' AND tp.onboard_ss_id = ?
    ORDER BY urs.id DESC
");
$s->bind_param('ss', $ss_key, $ss_key);
$s->execute();
$cns = $s->get_result()->fetch_all(MYSQLI_ASSOC);
$s->close();
?>
<!DOCTYPE html>
<html lang="en">
