...
// Resolve: if returnid given, get inv from return master — scoped to CNs
// against THIS SS's own issued invoices only.
if ($returnid) {
    $s = $db_conn->prepare("SELECT urs.*, tpi.id AS tpi_id, tpi.territory_partner_id, tp.name AS tp_name, tp.tp_id AS tp_code FROM user_return_stock urs JOIN tp_invoices tpi ON tpi.invoice_number = urs.invnumber COLLATE utf8mb4_unicode_ci JOIN territory_partners tp ON tp.id = tpi.territory_partner_id WHERE urs.returnid=? AND urs.from_usertype='territory_partner' AND urs.to_usertype='super_stockiest' AND urs.to_userid=? AND tpi.created_by_user_type='super_stockiest' AND tp.onboard_ss_id=? LIMIT 1");
    $s->bind_param('sss', $returnid, $ss_key, $ss_key);
    $s->execute();
...
$s = $db_conn->prepare("SELECT tpi.*, tp.name AS tp_name, tp.tp_id AS tp_code, tp.id AS tp_db_id FROM tp_invoices tpi JOIN territory_partners tp ON tp.id = tpi.territory_partner_id WHERE tpi.id=? AND tpi.created_by_user_type='super_stockiest' AND tp.onboard_ss_id=? LIMIT 1");
...
<head>
    ...
    <title>TP Return Invoice : <?php echo $business_name; ?></title>
