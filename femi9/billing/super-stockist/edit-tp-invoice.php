...
// Load invoice — ownership via TP.onboard_ss_id
$s = $db_conn->prepare("
    SELECT tpi.*, tp.name AS tp_name, tp.tp_id AS tp_code
...
// Get current available qty (SS's own stock) for each existing item's product
$avail_map = [];
foreach ($existing_items as $it) {
    $pid = (int)$it['product_id'];
    $sq = $db_conn->prepare("SELECT closing_qty FROM stock WHERE user_type='super_stockiest' AND user_id=? AND product_id=?");
    $sq->bind_param("si", $ss_id, $pid); $sq->execute();
...
(function ($) {
    ...
    function fetchBalance() {
        $.getJSON('get-tp-advance-balance.php?tp_id=' + tpId, function (res) {
            ...
            if (effective <= 0) {
                setBalancePanel('danger', '₹' + fmtAmt(effective), 'No available balance after restoring current invoice amount.');
            } else {
                setBalancePanel('ok', '₹' + fmtAmt(effective), 'Effective balance = current ₹' + fmtAmt(current) + ' + this invoice ₹' + fmtAmt(oldSubtotal) + ' (to be restored on save).');
            }
            updateSummary();
        }).fail(function () {
            advanceBalance = 0;
            setBalancePanel('warn', '—', 'Could not load balance. Balance will be checked on save.');
            updateSummary();
        });
    }

    function setBalancePanel(state, amount, note) {
    ...
    function loadProducts() {
        ...
        $.getJSON('get-ss-tp-products.php?tp_id=' + tpId, function (data) {
            ...
            $.each(data, function (_, p) {
                ...
                // Effective available = SS's current stock + what we already allocated in existing items
