...
<head>
    ...
    <title>Add TP Payment Entry : <?php echo $business_name; ?></title>
    ...
    <style>
        ...
        .form-label { font-weight:500; color:#2c3e50; margin-bottom:8px; }
        .form-control, .form-select { border-radius:6px; border:1px solid #d1d5db; padding:10px 12px; transition:all 0.3s; }
        .form-control:focus, .form-select:focus { border-color:#3b82f6; box-shadow:0 0 0 3px rgba(59,130,246,0.1); outline:none; }
        .btn-primary { background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); border:none; padding:12px 24px; border-radius:6px; font-weight:500; }
        .required-field::after { content:" *"; color:#ef4444; }
        .info-box { background:#eff6ff; border-left:4px solid #3b82f6; padding:12px 16px; border-radius:6px; margin-bottom:20px; }
        .user-info-badge { display:inline-block; background:#f3f4f6; padding:8px 16px; border-radius:20px; font-size:14px; color:#374151; margin-bottom:20px; }
        .select2-container--default .select2-selection--single { border-radius:6px; border:1px solid #d1d5db; height:auto; padding:10px 12px; font-size:1rem; }
        ...
        .select2-dropdown { border:1px solid #d1d5db; border-radius:6px; box-shadow:0 4px 16px rgba(0,0,0,0.12); }
                            ...
                            <div class="page-description">
                                <h1>
                                    <table class="headertble"><tr>
                                        ...
                                        <td><a href="manage-tp-advance-payments" title="View Payments">&#9776;</a></td>
                                    </tr></table>
                                </h1>
                            </div>
                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <div class="info-box">
                                        ...
                                        <strong>Note:</strong> Record advance payments received from Territory Partners under your account.
                                    </div>

                                    <div class="user-info-badge">
                                        <i class="material-icons-outlined" style="vertical-align:middle;font-size:18px;">account_circle</i>
                                        <strong>Recorded By:</strong> <?php echo htmlspecialchars($logged_user_name); ?>
                                    </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-12 mb-3">
                                                ...
                                                <textarea name="remarks" id="remarks" class="form-control"
                                                          rows="3" placeholder="Additional notes (optional)" maxlength="1000"></textarea>
                                            </div>
...
$(function () {
    ...
    // Pre-select TP if redirected from add-tp-invoice
    var preselect = <?php echo json_encode((int)($_GET['tp_id'] ?? 0)); ?>;
    if (preselect > 0) {
        $('#territory_partner_id').val(preselect).trigger('change');
    }
});

function validateForm() {
    var amount = parseFloat(document.getElementById('amount').value);
    if (!amount || amount <= 0) { alert('Please enter a valid amount greater than 0'); return false; }
    if (amount > 10000000 && !confirm('Amount is more than ₹1 Crore. Are you sure?')) return false;
