...
try {
    ...
    // For non-Super Stockist, company_id is required
    ...
    // Company-specific filter OR Super Stockist filter
    if ($logged_user_type === 'company') {
        $where_conditions[] = "u.onboard_userTYPE = 'company'";
        $where_conditions[] = "u.onboard_userID = 'company'";
    } elseif ($logged_user_type === 'super_stockiest') {
        // For Super Stockist: Only show stockists under them
        // Using both ss_id and onboard fields for compatibility
        $where_conditions[] = "(u.ss_id = '{$_SESSION['LOGIN_USER_ID']}' OR (u.onboard_userID = '{$_SESSION['LOGIN_USER_ID']}' AND u.onboard_userTYPE = 'super_stockiest'))";
    }
