...
// See tp-cnote-manage.php for why ownership is scoped via the TP's own
// onboard_ss_id rather than tp_invoices.created_by_user_id.
$ss_key = (string)$Login_user_IDvl;
...
// Load TP invoice — only if THIS SS issued it.
$s = $db_conn->prepare("SELECT tpi.*, tp.id AS tp_db_id FROM tp_invoices tpi JOIN territory_partners tp ON tp.id=tpi.territory_partner_id WHERE tpi.id=? AND tpi.created_by_user_type='super_stockiest' AND tp.onboard_ss_id=? LIMIT 1");
$s->bind_param('is', $inv_id, $ss_key);
$s->execute();
...
if (!$returnid) {
    ...
    $returnid = 'TPCN-' . $rand;
    $s = $db_conn->prepare("INSERT INTO user_return_stock (returnid, invnumber, date, from_usertype, from_userid, to_usertype, to_userid, subtotal, discount, total, status, rwpoints_enable, buyer_gsttype, gst_type) VALUES (?, ?, ?, 'territory_partner', ?, 'super_stockiest', ?, 0, 0, 0, 'pending', 0, '', '')");
    $tp_id_str = (string)$tp_db_id;
    $s->bind_param('sssss', $returnid, $inv_number, $today, $tp_id_str, $ss_key);
    $s->execute(); $s->close();
} else {
    // Verify it exists, is still pending, and belongs to this SS
    $s = $db_conn->prepare("SELECT id FROM user_return_stock WHERE returnid=? AND to_usertype='super_stockiest' AND to_userid=? AND status='pending' LIMIT 1");
    $s->bind_param('ss', $returnid, $ss_key);
    $s->execute();
