...
xmlhttp.open("GET","loadPriceOther.php?q="+str + "&invuser=customer",true);
xmlhttp.send();}
</script>
...
mysqli_query($db_conn,$update_zero_invoice);		
  ...
  function receiptamount(){
   ...
   document.getElementById('receivableamount').value = (totalbillamount*1)-(receivedamount*1); 
	...
	<input type="hidden" name="receivedamount" value="<?=$result_ReceiptDetails['received'];?>">
...
<p><b>Received Amount</b>
		 <input type="number" min="0" required="" step="any" id="receivedamount" class="form-control" style="width:100%;" onkeyup="receiptamount()" name="receivedamount">
		 </p>
...
flatpickr("#bookingDate", {
    ...
    <div class="item">
	<select required="" style="width:100%;" name="pr_id" required="" onChange="showPrice(this.value)" class="prinput">
<option value="" hidden="">Select Product</option>
<?php $select_Products_list="select * from products order by id asc";
$fetch_Products_list=mysqli_query($db_conn,$select_Products_list);
while($result_Products_list=mysqli_fetch_array($fetch_Products_list))
