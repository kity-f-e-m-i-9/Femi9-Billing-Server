...
// tp_invoices.created_by_user_id. Also doubles as the id the `stock` table
// keys super_stockiest rows on.
...
// Load CN master — scoped to CNs against THIS SS's own issued invoices only.
$s = $db_conn->prepare("SELECT status, from_userid, invnumber FROM user_return_stock WHERE returnid=? AND from_usertype='territory_partner' AND to_usertype='super_stockiest' AND to_userid=? LIMIT 1");
...
if ($cn['status'] === 'accept') {
    ...
    // Determine original source — must still belong to this SS.
    $s = $db_conn->prepare("SELECT tpi.id, tpi.source_godown_id, tpi.source_cp_id, tpi.source_location_id, tpi.warehouse_id FROM tp_invoices tpi JOIN territory_partners tp ON tp.id = tpi.territory_partner_id WHERE tpi.invoice_number=? AND tpi.created_by_user_type='super_stockiest' AND tp.onboard_ss_id=? LIMIT 1");
    ...
    $use_godown       = (!$use_ss_stock && $source_godown_id > 0 && !$source_cp_id);
    ...
    try {
        ...
        if ($use_ss_stock) {
            $s = $db_conn->prepare("SELECT closing_qty FROM stock WHERE user_type='super_stockiest' AND user_id=? AND product_id=? FOR UPDATE");
            ...
            $s = $db_conn->prepare("UPDATE stock SET sent_qty=sent_qty+?, returnqty=GREATEST(0,returnqty-?), closing_qty=closing_qty-? WHERE user_type='super_stockiest' AND user_id=? AND product_id=?");
            $s->bind_param('iiisi', $qty, $qty, $qty, $ss_stock_id, $prid);
            $s->execute(); $s->close();

            $s = $db_conn->prepare("INSERT INTO stock_ledger (product_id, user_type, user_id, action, qty, qty_before, qty_after, ref_type, ref_id, note, created_by) VALUES (?,'super_stockiest',?,'transfer_out',?,?,?,'return',?,?,?)");
            $s->bind_param('isiiisss', $prid, $ss_stock_id, $qty, $src_before, $src_after, $returnid, $note, $created_by);
            $s->execute(); $s->close();
        ...
        $db_conn->rollback();
        error_log("[TP CN Item Del - SS] Failed: " . $e->getMessage());
