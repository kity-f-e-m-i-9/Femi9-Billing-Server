...
error_reporting(0);
	...
	echo "<script>window.location='stock_request_details.php?invoicealready&&reqid=".base64_encode($inv_id)."';</script>";
	}else{
...
{
	echo "<script>window.location='stock_request_details.php?reqid=".base64_encode($inv_id)."&&gid=".$godownid."&&stocknotupdated';</script>";
...
$sqladminlog="select * from admin_log";
...
//--------------------------------------------------------------------------------
	...
	if($AVMstock<$qty)
	{
		echo "<script>window.location='stock_request_details.php?reqid=".base64_encode($inv_id)."&&InvalidStock&&AlertStockError&&gid=".$godownid."';</script>";
	...
	if($result_count_invoiceItem['numInvoiceItem']==0)
	{
		...
		echo "<script>window.location='stock_request_details.php?reqid=".base64_encode($inv_id)."&&AddedSuccess&&FemiAdded&&gid=".$godownid."';</script>";
		
	}else{
		
		echo "<script>window.location='stock_request_details.php?reqid=".base64_encode($inv_id)."&&ItemAlreadyExists&&AlertMessage&&gid=".$godownid."';</script>";
	}
