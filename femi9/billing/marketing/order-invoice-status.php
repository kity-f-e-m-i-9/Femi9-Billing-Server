...
// Ownership check — a DM may view their own orders, or (if it belongs to
// someone in their downline) a manager may view a subordinate's order via
// the same view_ms_id scoping used on manage_order_product.php.
$owns = false;
if ($order_id !== '') {
    $stmtOwnerId = $db_conn->prepare("SELECT ms_id FROM ms_orders WHERE order_id=? LIMIT 1");
    $stmtOwnerId->bind_param('s', $order_id);
    $stmtOwnerId->execute();
    $orderOwnerMsId = (int)($stmtOwnerId->get_result()->fetch_assoc()['ms_id'] ?? 0);
    $stmtOwnerId->close();

    if ($orderOwnerMsId > 0) {
        if ($orderOwnerMsId === (int)$markeingSTFID) {
            $owns = true;
        } else {
            $allowedSubtree = getMsSubtreeIds($db_conn, (int)$markeingSTFID);
            $owns = in_array($orderOwnerMsId, $allowedSubtree, true);
        }
    }
}
...
if ($owns) {
    ...
    $stmtTp = $db_conn->prepare("SELECT invoiced_inv_id, voided_at FROM tp_orders WHERE order_id=? LIMIT 1");
    $stmtTp->bind_param('s', $order_id);
    $stmtTp->execute();
    $tpOrderRow = $stmtTp->get_result()->fetch_assoc();
    $invId = $tpOrderRow['invoiced_inv_id'] ?? null;
    $voidedBeforeInvoicing = !empty($tpOrderRow['voided_at']);
    $stmtTp->close();
            ...
            <div class="content-wrapper">
                <div class="container-fluid">
                    ...
                    <?php else: ?>
                        ...
                        <?php elseif ($voidedBeforeInvoicing): ?>
                        <div class="row mb-2">
                            <div class="col"><div class="alert alert-danger">The TP voided this order before invoicing it — the shop did not want this product. No stock was affected.</div></div>
                        </div>
