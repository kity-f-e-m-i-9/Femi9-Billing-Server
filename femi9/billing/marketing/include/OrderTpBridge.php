...
function bridgeOrderToTp($db_conn, int $tp_id, $ms_id, $shop_id, string $order_id, string $order_date, array $lines): void
{
    ...
    try {
        ...
        if ($found) {
            ...
            // Placeholder order: state_id, temp_id, name, district_id, mobile,
            // valid_from, valid_to, gstin, onboard_userID, address, userid,
            // useridtext, latitude, longitude, source_ms_shop_id
            $stmtIns->bind_param(
                'ississssssisddi',
                $state_id, $tempId, $name, $district_id, $mobile,
                $valid_from, $valid_to, $gstin, $tp_id_str,
                $address, $userid, $useridtext,
                $lat, $lng, $shop_id_int
            );
            $stmtIns->execute();
