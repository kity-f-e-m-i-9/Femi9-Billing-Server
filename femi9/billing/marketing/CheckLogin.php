...
try {
    ...
    $stmt = mysqli_prepare($db_conn, 
        "SELECT id, ms_name, password, ms_email, ms_mobile, account_status 
    ...
    try {
        ...
        if ($isPasswordValid) {
            ...
            $updateStmt = mysqli_prepare($db_conn,
                "UPDATE marketing_staff SET password = ? WHERE ms_mobile = ?"
            );
    ...
    $_SESSION['LOGIN_USER'] = $user['ms_mobile'];
    $_SESSION['LOGIN_USER_ID'] = $user['id'];
    $_SESSION['LOGIN_USER_NAME'] = $user['ms_name'];
    $_SESSION['LOGIN_USER_TYPE'] = 'marketing';
    $_SESSION['last_activity'] = time();
    
    // Update last login time
    $updateLoginStmt = mysqli_prepare($db_conn,
        "UPDATE marketing_staff SET last_login = NOW() WHERE ms_mobile = ?"
    );
