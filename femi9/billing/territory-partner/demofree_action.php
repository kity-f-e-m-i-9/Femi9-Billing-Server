...
// NOTE: These functions must be called inside an open transaction so FOR UPDATE is effective.

function tp_get_closing_qty_locked($db, $tp_id, $pid) {
    // SELECT ... FOR UPDATE — acquires row lock to prevent concurrent modifications
    $s = $db->prepare("SELECT closing_qty FROM territory_partner_stock WHERE territory_partner_id=? AND product_id=? FOR UPDATE");
...
function tp_deduct_stock($db, $tp_id, $pid, $qty, $ref_type, $ref_id, $created_by) {
    $before = tp_get_closing_qty_locked($db, $tp_id, $pid);
...
if (isset($_REQUEST['add-record'])) {
    ...
    try {
        ...
        foreach ($rows as $row) {
            ...
            $available = tp_get_closing_qty_locked($db_conn, $tp_id, $pid);
            if ($available < $qty) {
                $db_conn->rollback();
                $_SESSION['errorMessage'] = "Insufficient stock for one or more products. Available: $available, Requested: $qty.";
                header("Location: demofree-new.php?stockerror");
                exit;
            }
