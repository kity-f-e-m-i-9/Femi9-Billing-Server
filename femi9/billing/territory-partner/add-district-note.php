...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // A single assigned district is used automatically — never trust a
    // posted value for it. Only when there's genuinely more than one does
    // the picker exist at all, and even then every value must be one of
    // this TP's own assigned districts.
    if (count($districts) === 1) {
        $selectedDistricts = $districts;
    } else {
        $selectedDistricts = array_map('trim', (array)($_POST['district'] ?? []));
    ...
    // district this TP is actually assigned to.
    ...
    if (empty($selectedDistricts) || $invalidDistrict) {
        ...
        $photoPath = null;
        // Photo upload only applies to a Software Issue — a Field Issue
        // never shows that field at all, so ignore anything posted
        // alongside it.
        if ($noteType === 'software' && !empty($_FILES['photo']['name']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($file['size'] > 10 * 1024 * 1024) {
                ...
                $photoPath = 'note_' . $tp_id . '_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
        ...
        if ($errorMsg === '') {
            // One row per selected district — same issue/priority/photo, so
            // Manage Notes' per-district filter still finds it correctly
            // under each district it was logged against.
            $stmt = $db_conn->prepare(
                "INSERT INTO tp_district_notes (tp_id, district, note_type, issue_text, priority, photo_path) VALUES (?, ?, ?, ?, ?, ?)"
            );
            foreach ($selectedDistricts as $d) {
                $stmt->bind_param('isssss', $tp_id, $d, $noteType, $issueText, $priority, $photoPath);
                $stmt->execute();
            }
            $stmt->close();
...
<head>
    ...
    <style>
        ...
        /* Searchable multi-select district picker (select2), shown only when
           the TP has more than one assigned district — restyled to match the
           plain .form-control fields around it. */
            ...
            <div class="content-wrapper">
                <div class="container-fluid">
                            ...
                            <div class="page-description">
                                <h1>
                                    <table class="headertble">
                                        <tr>
                                            <td>Add District Note</td>
                                            <td><a href="manage-district-notes.php" title="Manage Notes">&#9776;</a></td>
                                        </tr>
                                    </table>
                                </h1>
                            </div>
                    ...
                    <?php if (empty($districts)): ?>
                        <div class="alert alert-info">No district is assigned to you yet.</div>
                    <?php else: ?>
                            ...
                            <div class="dn-field">
                                ...
                                <div class="dn-type-switch" id="noteTypeSwitch">
                                    <label class="dn-type-btn <?php echo $postedNoteType === 'tp' ? 'active' : ''; ?>" data-val="tp">
                                        ...
                                        <i class="material-icons-outlined">storefront</i> Field Issue
                                    </label>
                            ...
                            <div class="dn-field">
                                ...
                                <?php if (count($districts) === 1): ?>
                                    <div class="dn-fixed-district">
                                        ...
                                        <span><?php echo htmlspecialchars($districts[0]); ?></span>
                                    </div>
                                    <input type="hidden" name="district[]" value="<?php echo htmlspecialchars($districts[0], ENT_QUOTES); ?>">
                                <?php else: ?>
