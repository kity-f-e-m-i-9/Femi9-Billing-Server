...
try {
    ...
    if (!empty($_REQUEST['update_invoice_date'])) {
        ...
        $s = $db_conn->prepare("UPDATE user_invoice SET date=? WHERE inv_id=? AND from_user_type=? AND from_user_id=?");
        $s->bind_param('sssi', $upd_date, $invoice_id, $Login_user_TYPEvl, $tp_id);
        $s->execute(); $s->close();
        $s = $db_conn->prepare("UPDATE user_invoice_items SET date=? WHERE inv_id=? AND from_user_type=? AND from_user_id=?");
        $s->bind_param('sssi', $upd_date, $invoice_id, $Login_user_TYPEvl, $tp_id);
        $s->execute(); $s->close();
    }

    $s = $db_conn->prepare("SELECT * FROM user_invoice WHERE inv_id=? LIMIT 1");
    ...
    $s = $db_conn->prepare("SELECT pr_id, qty FROM user_invoice_items WHERE inv_id=? AND from_user_type=? AND from_user_id=?");
    ...
    $s = $db_conn->prepare(
        "UPDATE user_invoice SET sub_total=?,discount=?,total=?,roundoff=?,courier_charges=?
    ...
    $from_uid        = $inv['from_user_id']   ?? (string)$Login_user_IDvl;
    ...
    header("Location: shop-manage-invoice.php?submiterror");
    exit;
}
