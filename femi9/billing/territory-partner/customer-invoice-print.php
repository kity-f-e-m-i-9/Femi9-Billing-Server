...
<td><button type="button" id="waShareBtn" onclick="shareInvoiceToWhatsApp({elementId:'divToPrint', mobile:'<?php echo htmlspecialchars($buyer['mobile'] ?? '', ENT_QUOTES); ?>', invoiceNumber:'<?php echo htmlspecialchars($inv['inv_number'] ?? '', ENT_QUOTES); ?>', fileName:'Invoice_<?php echo htmlspecialchars($inv['inv_number'] ?? '', ENT_QUOTES); ?>', businessName:'<?php echo htmlspecialchars($seller_display_name, ENT_QUOTES); ?>', button:this});" class="btn btn-success m-b-xs m-r-xs"><i class="material-icons" style="font-size:16px;vertical-align:middle;">share</i> Share to WhatsApp</button></td>
<td><button type="button" onClick="javascript:window.location='customer-invoice-add.php';" class="btn btn-success m-b-xs m-r-xs">+ New Invoice</button></td>
<td><button type="button" onClick="javascript:window.location='customer-manage-invoice.php';" class="btn btn-primary m-b-xs m-r-xs">Manage Invoice</button></td>
</tr>
</table>
...
$fetch_INVProductDetails = mysqli_query($db_conn, "SELECT * FROM invoice_items WHERE inv_id='$Invoice_ID' ORDER BY id DESC");
while ($result_INVProductDetails = mysqli_fetch_array($fetch_INVProductDetails)) {
    $InV_Product_ID = $result_INVProductDetails['pr_id'];
    $result_ProductDetails123 = mysqli_fetch_array(mysqli_query($db_conn, "SELECT * FROM products WHERE id='$InV_Product_ID' LIMIT 1"));
...
$totalgstamount = mysqli_fetch_array(mysqli_query($db_conn, $select_sum_gstamount))[0];
...
while ($resulthsn = mysqli_fetch_array($fetchhsn)) {
    ...
    $resulthsnTaxamount = mysqli_fetch_array(mysqli_query($db_conn, "SELECT SUM(total) FROM invoice_items WHERE inv_id='$Invoice_ID' AND hsn='$hsncode'"));
?>
<tr>
