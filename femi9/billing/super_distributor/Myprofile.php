...
//---------------------------------------------------------------------------------------
//user details
$select_userdetails234="select * from super_distributor where temp_id='$Login_user_IDvl'";
...
if($filetype12 != 'image/jpeg' && $filetype12 != 'image/jpg' && $filetype12 != 'image/png')
{
 ...
 echo "<script>window.location='Myprofile?uploadError';</script>";
	   ...
	   unlink("../super_distributor/".$old_user_icon."");
	   
}

	}else{$insfilename=$old_user_icon;}
	
	$updateusers="update super_distributor set user_icon='$insfilename',name='$name',email='$email',gstin='$gstin',address='$address',state_id='$state_id',district_id='$district_id',taluk_id='$taluk_id',pincode_id='$pincode_id' where temp_id='$user_tempid'";
...
<input type="hidden" name="usertype" value="<?=$Login_user_TYPEvl?>">
			...
			<img src="../super_distributor/<?=$result_userdetails234['user_icon'];?>" style="width:150px;margin-bottom:10px;margin-top:10px;"/>
