...
$temp_time=date("gis"); 
	...
	if($result_count_product['numProducts']==0)
	{
		...
		echo "<script>window.location='manage-coupon.php?addedsuccess';</script>";
		
	}else{
		echo "<script>window.location='coupon.php?alreadyexists';</script>";
	}
...
if(isset($_REQUEST['update-product']))
{
	...
	mysqli_query($db_conn,$update_products);
		...
		echo "<script>window.location='manage-coupon.php?updatedSuccess';</script>";
