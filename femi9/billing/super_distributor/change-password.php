...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    try {
        ...
        $stmt = mysqli_prepare($db_conn, 
            "SELECT password FROM super_distributor WHERE username = ? LIMIT 1"
        );
        ...
        try {
            ...
            $updateStmt = mysqli_prepare($db_conn,
                "UPDATE super_distributor SET password = ? WHERE username = ?"
            );
            ...
            $clearFlagStmt = mysqli_prepare($db_conn,
                "UPDATE forgotpassword 
                 ...
                 WHERE usertype = 'super_distributor' AND mobilenumber = ?"
            );
