                        ...
                        <div class="row">
                            <div class="col-md-12">
								...
								<?php if(isset($_REQUEST['DeleteSuccess'])){?><div class="alert alert-danger">Deleted ! one product deleted success.</div><?php }?>
									...
									<?=$displaytitle;?></td>
									<td><a href="user-manage-invoice.php?invuser=<?=$_REQUEST['invuser'];?>" title="Manage Invoice">&#9776;</a></td>
									</tr>
									</table>
									</h1>
...
<input type="number" min="0" name="amount" step="any" onKeyup="totalkm()" id="amount" placeholder="Price"></span>
		...
		<input type="number" min="0" step="any" name="total" id="output" readonly class="numberinput" required="" readonly placeholder="Total">
...
<?php } else{ echo "<span id='cnlable'>-&nbsp;CN&nbsp;-</span>";}?>
  ...
  function totalamount(){
   ...
   document.getElementById('outputTotalamount').value = (subtotal*1)-(discount*1); 
                                            ...
                                            <div class="col-lg-3">
                                                <div class="invoice-info">
<form action="user-invoice-submit.php" method="post" enctype="multipart/form-data" onSubmit="return confirm('Please make a confirm!');">
...
<input type="hidden" name="SubTotal" value="<?=$TotalAMount123;?>"/>
            ...
            <p><b>Sub Total</b>
			<input type="number" step="any" class="form-control" min="0" value="<?=$TotalAMount123;?>" style="width:100%;" id="subtotal" disabled></span>
			</p>
...
xmlhttp.send();}
			...
			<script>
			// Auto-suggest next invoice number (per downstream target: DT/FY/LLP{n}
			// napkin / DTD/FY/LLP{n} diaper for the distributor network target,
			// SDSH/FY/LLP{n} napkin / SDSHD/FY/LLP{n} diaper for the shop target).
			// Suggested on page load (napkin default, since no product is picked
			// yet) and re-suggested once a product is selected via showPrice()
			// below — see invNumberSuggest(). Only ever fills/overwrites while
			// the field still holds an auto-suggested value; stops touching it
			// the moment the user types anything themselves.
...
<?php if($amount_received_fully==0){?>
<div class="item">
<select required="" name="pr_id" style="width:100%;" required="" onChange="showPrice(this.value); invNumberSuggest(this.value);" class="prinput">
<option value="" hidden="">Select Product</option>
<?php $select_Products_list="select * from products order by id asc";
$fetch_Products_list=mysqli_query($db_conn,$select_Products_list);
while($result_Products_list=mysqli_fetch_array($fetch_Products_list))
...
<input type="number" min="0" name="qty" id="qty" onKeyup="totalkm()" required="" placeholder="Qty" class="numberinput">
<span id="txtHintPrice">
<input type="number" min="0" name="amount" step="any" onKeyup="totalkm()" id="amount"  placeholder="Price">
</span>

		<input type="number" min="0" step="any" name="total" id="output" required="" readonly placeholder="Total" class="numberinput">
