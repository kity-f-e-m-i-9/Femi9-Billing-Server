...
try {
    ...
    try {
        ...
        if ($isPasswordValid) {
            ...
            $updateStmt = mysqli_prepare($db_conn,
                "UPDATE super_distributor SET password = ? WHERE mobile_number = ?"
            );
    ...
    $_SESSION['LOGIN_USER_TYPE'] = 'super_distributor';
    $_SESSION['last_activity'] = time();
    
    // Update last login time
    $updateLoginStmt = mysqli_prepare($db_conn,
        "UPDATE super_distributor SET last_login = NOW() WHERE mobile_number = ?"
    );
